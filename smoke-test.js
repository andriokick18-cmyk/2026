...
(async () => {
  ...
  try {
    ...
    {
      await req2("POST", "/api/test/login", { token: TEST_TOKEN, email: "smoke@test.com", isAdmin: true });
      const _mig0 = await get("/api/admin/migracao-codigos");
      check("🎟️ v206: servidor recém-nascido tem ZERO códigos de migração — nenhum dado de cliente vem embutido no repositório (que é PÚBLICO); os códigos só nascem pelo painel",
        _mig0.status === 200 && Array.isArray(_mig0.json?.codigos) && _mig0.json.codigos.length === 0 && !fs.existsSync(path.join(DATA, "migration_codes.json")),
        JSON.stringify(_mig0.json));
      const _migC1 = await req2("POST", "/api/admin/migracao-codigos", { nome: "Cliente Migrado", email: "Migrado@gmail.com", plano: "vipro", dias: 25 });
      const _cod1 = _migC1.json?.criados?.[0]?.codigo;
      check("🎟️ v206: admin cria 1 código pelo painel — gerado no formato H2B26-XXXX-XXXX (alfabeto sem 0/O/1/I, é ditado por WhatsApp), e-mail normalizado em minúsculas",
        _migC1.status === 200 && /^H2B26-[A-HJ-NP-Z2-9]{4}-[A-HJ-NP-Z2-9]{4}$/.test(_cod1 || "") && _migC1.json.criados[0].email === "migrado@gmail.com" && _migC1.json.erros.length === 0,
        JSON.stringify(_migC1.json));
      const _migC2 = await req2("POST", "/api/admin/migracao-codigos", { itens: [
        { nome: "Segundo", email: "migrado2@gmail.com", plano: "doublepro", dias: 7, codigo: "H2B26-TEST-0002" },
        { nome: "Errado", email: "sem-arroba", plano: "vipro", dias: 5 },
        { nome: "Errado2", email: "x@gmail.com", plano: "ouro", dias: 5 },
        { nome: "Errado3", email: "y@gmail.com", plano: "vip", dias: 0 },
      ] });
      check("🎟️ v206: importação em lote — a linha certa entra COM o código informado; cada linha errada (e-mail, plano, dias) é recusada com o motivo sem derrubar as outras",
        _migC2.status === 200 && _migC2.json?.criados?.length === 1 && _migC2.json.criados[0].codigo === "H2B26-TEST-0002" && _migC2.json.erros.length === 3 && _migC2.json.erros.map((e) => e.linha).join(",") === "2,3,4",
        JSON.stringify(_migC2.json));
      const _migDup = await req2("POST", "/api/admin/migracao-codigos", { nome: "Dup", email: "dup@gmail.com", plano: "vip", dias: 3, codigo: "h2b26-test-0002" });
      check("🎟️ v206: código repetido (mesmo em caixa diferente) é recusado — nunca 2 clientes com o mesmo código",
        _migDup.status === 200 && _migDup.json?.criados?.length === 0 && /já existe/.test(_migDup.json?.erros?.[0]?.erro || ""), JSON.stringify(_migDup.json));
      const _finAntes = ((await get("/api/admin/financeiro")).json?.pagamentos || []).length;
      // Conta ERRADA tenta o código de outra pessoa
      await req2("POST", "/api/test/login", { token: TEST_TOKEN, email: "intruso@gmail.com" });
      const _migF = await req2("POST", "/api/vip/resgatar-codigo", { codigo: _cod1 });
      check("🎟️ v206: o código é PRESO ao Gmail — outra conta leva 403 e o código continua disponível pro dono", _migF.status === 403, JSON.stringify(_migF.json));
      const _migAnon = await new Promise((resolve) => { const r = http.request(BASE + "/api/vip/resgatar-codigo", { method: "POST", headers: { "Content-Type": "application/json" } }, (res) => { res.resume(); res.on("end", () => resolve({ status: res.statusCode })); }); r.end(JSON.stringify({ codigo: _cod1 })); });
      check("🎟️ v206: sem sessão → 401 (um código nunca vira ativação anônima)", _migAnon.status === 401, JSON.stringify(_migAnon));
      // Conta CERTA — o e-mail da conta é o da tabela
      await req2("POST", "/api/test/login", { token: TEST_TOKEN, email: "migrado@gmail.com", name: "Cliente Migrado" });
      const _stAntes = (await get("/api/status")).json;
      const _migOk = await req2("POST", "/api/vip/resgatar-codigo", { codigo: " " + _cod1.toLowerCase() + " " });
      const _stDepois = (await get("/api/status")).json;
      const _DAYm = 86400_000, _agoraM = Date.now();
      check("🎟️ v206: resgate na conta certa (código aceito em minúsculas e com espaços) → 200; o /api/status vira VIPro ATIVO com os limites da tabela NOVA (100/100) e source 'migracao'",
        _migOk.status === 200 && _migOk.json?.ok === true && _migOk.json.dias === 25 && _migOk.json.plano === "vipro" &&
        _stAntes.needsPlan === true && _stDepois.needsPlan === false && _stDepois.plan === "vipro" && _stDepois.vip?.active === true && _stDepois.vip.source === "migracao" &&
        _stDepois.manualLimit === 100 && _stDepois.autoLimit === 100,
        JSON.stringify({ r: _migOk.json, antes: _stAntes.needsPlan, plan: _stDepois.plan, ml: _stDepois.manualLimit, al: _stDepois.autoLimit, src: _stDepois.vip?.source }));
      check("🎟️ v206: os 25 dias entram nos DOIS relógios (VIPro tem manual e automático), contando de hoje",
        Math.abs((_stDepois.vip?.manualExpires || 0) - (_agoraM + 25 * _DAYm)) < 5 * 60_000 && Math.abs((_stDepois.vip?.autoExpires || 0) - (_agoraM + 25 * _DAYm)) < 5 * 60_000,
        JSON.stringify({ m: _stDepois.vip?.manualExpires, a: _stDepois.vip?.autoExpires, agora: _agoraM }));
      check("🎟️ v206: conta migrada é plano PAGO pra tudo que é feature — VIPro libera 1 Gmail de envio (getMaxSenders não a trata como cortesia/trial)",
        _stDepois.senderMax === 1, JSON.stringify({ senderMax: _stDepois.senderMax }));
      const _migRe = await req2("POST", "/api/vip/resgatar-codigo", { codigo: _cod1 });
      const _migInv = await req2("POST", "/api/vip/resgatar-codigo", { codigo: "H2B26-NAOE-XIST" });
      const _migVazio = await req2("POST", "/api/vip/resgatar-codigo", { codigo: "" });
      check("🎟️ v206: uso único — repetir dá 409; código inexistente 404; vazio 400",
        _migRe.status === 409 && _migInv.status === 404 && _migVazio.status === 400, JSON.stringify([_migRe.status, _migInv.status, _migVazio.status]));
      // Conta NOVA de verdade (chave = username, Gmail confirmado em emailContato — cadastro v175)
      ...
      await req2("POST", "/api/test/login", { token: TEST_TOKEN, email: "smoke@test.com", isAdmin: true });
      const _finDepois = ((await get("/api/admin/financeiro")).json?.pagamentos || []).length;
      check("🎟️ v206: ZERO receita nova — o caixa tem exatamente o mesmo nº de lançamentos antes e depois dos 2 resgates (o dinheiro entrou no site antigo)", _finDepois === _finAntes, JSON.stringify({ antes: _finAntes, depois: _finDepois }));
      const _migL = (await get("/api/admin/migracao-codigos")).json;
      const _l1 = (_migL?.codigos || []).find((c) => c.codigo === _cod1), _l2 = (_migL?.codigos || []).find((c) => c.codigo === "H2B26-TEST-0002");
      check("🎟️ v206: a lista do painel mostra quem usou e quando (usadoPor = a conta que resgatou)", _migL?.usados === 2 && _l1?.usado === true && _l1.usadoPor === "migrado@gmail.com" && !!_l1.usadoEm && _l2?.usado === true && _l2.usadoPor === "conta.nova@test.com", JSON.stringify(_migL));
      const _migC3 = await req2("POST", "/api/admin/migracao-codigos", { nome: "Terceiro", email: "migrado3@gmail.com", plano: "vip", dias: 10 });
      const _cod3 = _migC3.json?.criados?.[0]?.codigo;
      const _delUsado = await req2("POST", "/api/admin/migracao-codigos/excluir", { codigo: _cod1 });
      const _delLivre = await req2("POST", "/api/admin/migracao-codigos/excluir", { codigo: _cod3 });
      const _delNada = await req2("POST", "/api/admin/migracao-codigos/excluir", { codigo: "H2B26-NAOE-XIST" });
      const _migL2 = (await get("/api/admin/migracao-codigos")).json;
      check("🎟️ v206: excluir só código SEM uso (usado fica no histórico → 400; livre → 200 e some; inexistente → 404)",
        _delUsado.status === 400 && _delLivre.status === 200 && _delNada.status === 404 && !(_migL2?.codigos || []).some((c) => c.codigo === _cod3) && (_migL2?.codigos || []).some((c) => c.codigo === _cod1),
        JSON.stringify([_delUsado.status, _delLivre.status, _delNada.status]));
      const _migDisco = JSON.parse(fs.readFileSync(path.join(DATA, "migration_codes.json"), "utf8"));
      check("🎟️ v206: o uso é gravado no disco NA HORA (persist síncrono — é acesso/dinheiro, regra 6e), não num debounce que um deploy pode perder", !!_migDisco?.[_cod1]?.usadoEm && _migDisco[_cod1].usadoPor === "migrado@gmail.com", JSON.stringify(Object.keys(_migDisco)));
      const _aud = (await get("/api/admin/audit")).json?.audit || [];
      check("🎟️ v206: criar/excluir código deixa trilha no Audit Log do admin (quem, quando, o quê)", _aud.some((a) => a.action === "migracao_codigo_create") && _aud.some((a) => a.action === "migracao_codigo_delete"), JSON.stringify(_aud.slice(0, 3).map((a) => a.action)));
      const _srv206 = fs.readFileSync(path.join(__dirname, "server.js"), "utf8");
      const _idx206 = fs.readFileSync(path.join(__dirname, "index.html"), "utf8");
      const _adm206 = fs.readFileSync(path.join(__dirname, "admin.html"), "utf8");
      check("🎟️ v206 (estrutural, guarda permanente): NENHUM código/cliente vem hardcoded no server.js (repositório público — nome e Gmail de cliente nunca entram no git); a tela do usuário e a aba do painel existem",
        !/DB_MIGRACAO_CODES\s*\[\s*["'][A-Z0-9-]+["']\s*\]\s*=/.test(_srv206) && !/H2B26-(?!XXXX)[A-Z0-9]{4}-(?!XXXX)[A-Z0-9]{4}/.test(_srv206) &&
