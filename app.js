...
function renderAutoSenders(){
  ...
  // 🚨 v172l (ordem do dono, 13/09/2026 — "deixa claro que só funciona com
  // mais de 1 e-mail dependendo da conta"): quantos e-mails cabem é do
  // PLANO (free:1 · VIP/VIPro/DoublePro:2 · admin:6 — getMaxSenders no
  // servidor), não um recurso "de admin". Legenda sempre visível diz o
  // teto real de QUEM está logado, pra ninguém achar que é bug quando só
  // vê 1 e-mail (free) ou 2 (pago), nem confundir "meu" limite com o do
  // admin (6).
  const hint=g("#auto-senders-hint");
  const max=U.senderMax||1;
  if(hint)hint.textContent=U.isAdmin
    ?`Conta admin: até ${max} e-mails de envio (revezamento automático).`
    :max>1
      ?`Seu plano permite até ${max} e-mails de envio — conecte um extra no Perfil pra reduzir risco de bloqueio.`
      :`Seu plano permite 1 e-mail de envio (o principal). Planos VIP+ liberam um 2º Gmail extra.`;
...
function _renderPlanosUI(d){
  ...
  box.innerHTML=['vip','vipro','doublepro'].map(pl=>{
    ...
    return `<div style="background:var(--surface);border:2px solid ${isSel?'var(--blue)':'var(--border2)'};border-radius:var(--rl);padding:14px;margin-bottom:10px;${isSel?'box-shadow:0 0 0 3px rgba(37,99,235,.14)':''}">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px">${manualBadge}${autoBadge}</div>
