...
const LANG_DICT = {
  ...
  en: {
    ...
    "au_t":"One-account rule — read before signing in","au_b":"Each person may have ONE H2BApply account. Creating a second account — even with a different e-mail — can lead to a PERMANENT BAN of both accounts, with nothing refunded. Remember: the name on your resume is always the same, and the system cross-checks name, phone and device on its own — a duplicate account is easy to detect. Access spots are limited: always use the SAME e-mail, and on the Google screen pick exactly the e-mail you typed.","au_f":"✅ 1 person = 1 account = all your sends and VIP days always together and safe.", // ⚠️ v149: regra de conta única (ordem do dono)
    "rn_t":"Important update — read before signing in","rn_p1":"We updated H2BApply: the 3 servers became ONE — every account was brought here. NOTHING was deleted: your sends, VIP days and resumes are all here. Sign in using only 1 e-mail, preferably the main e-mail already registered on your account.","rn_warn":"⚠️ Anti-duplication system: if 2 accounts are created with the same name on the resume — even with different e-mails — both accounts can be PERMANENTLY BANNED. Use only 1 account.","rn_ok":"✅ I read and understood — continue", // 📢 v150: janela do reset antes do login
  ...
  es: {
    ...
    "rst_t":"Actualizamos el sistema de inicio de sesión","rst_b":"Tu inicio de sesión fue reiniciado, pero <strong>NADA fue borrado</strong> — tus postulaciones, días de plan y currículums están guardados. Entra con el <strong>MISMO correo de Google</strong> que usabas antes. ⚠️ <strong>No crees otra cuenta</strong> con un correo diferente: más de una cuenta arriesga un <strong>baneo permanente de ambos correos</strong>. Los cupos son limitados — usa la cuenta que ya tienes.", // 📢 v144: aviso do reset de login (OAuth novo)
    ...
    "au_t":"Regla de cuenta única — lee antes de entrar","au_b":"Cada persona puede tener UNA cuenta en H2BApply. Crear una segunda cuenta — incluso con otro e-mail — puede causar un BAN PERMANENTE de las dos cuentas, sin devolución de nada. Recuerda: el nombre en tu currículum es siempre el mismo, y el sistema cruza nombre, teléfono y dispositivo por sí solo — una cuenta duplicada es fácil de detectar. Los cupos de acceso son limitados: usa siempre el MISMO e-mail, y en la pantalla de Google elige exactamente el e-mail que escribiste.","au_f":"✅ 1 persona = 1 cuenta = todos tus envíos y días de VIP siempre juntos y seguros.", // ⚠️ v149: regra de conta única (ordem do dono)
    "rn_t":"Actualización importante — lee antes de entrar","rn_p1":"Actualizamos H2BApply: los 3 servidores se volvieron UNO solo — todas las cuentas fueron traídas aquí. NADA fue borrado: tus envíos, días de VIP y currículums están aquí. Entra usando solo 1 e-mail, de preferencia el e-mail principal que ya estaba registrado en tu cuenta.","rn_warn":"⚠️ Sistema anti-duplicación: si se crean 2 cuentas con el mismo nombre en el currículum — incluso con e-mails diferentes — las dos cuentas pueden ser BANEADAS PERMANENTEMENTE. Usa solamente 1 cuenta.","rn_ok":"✅ Leí y entendí — continuar", // 📢 v150: janela do reset antes do login
...
async function loadMeusPagamentos(){
  ...
  try{
    ...
    if(!peds.length){
      box.innerHTML=codeNote+`<div style="font-size:12px;color:var(--t3);text-align:center;padding:10px">Nenhum pedido por aqui ainda.</div>`;
      return;
    }
