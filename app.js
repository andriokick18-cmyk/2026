...
/* ⭐ v134: FUNIL — limite diário atingido = mostrar o que ele está perdendo
   ...
   está ativo — v172i), nunca por "tem plano":
     ...
     · só manual ativo  → VIPro/DoublePro somam o robô ao que ele já tem;
     · só o automático  → o manual dele venceu: a data é o que importa, não
                          um upgrade de tier;
     · DoublePro (topo) → NENHUM upsell: não existe pra onde subir. */
function limitUpsell(){
  ...
  if(!_manualOn&&!_autoOn){
    ...
    _corpo=esc(`Você usou os ${U.manualLimit} envios manuais de hoje. Os planos com robô (VIPro e DoublePro) seguem enviando por você 24h, sem você precisar abrir o app.`);
...
function _renderProfileSenderSection(){
  ...
  // 📧 ORDEM DO DONO (13/09/2026): Gmail extra é SÓ DoublePro (2 e-mails);
  // VIP/VIPro têm 1 (o principal); sem plano, 0. senderMax vem do servidor
...
// que bloqueiam envio — nos demais (admin liberado, VIPro/DoublePro com
...
function renderAutoSenders(){
  ...
  // grátis 0 · VIP/VIPro 1 · DoublePro 2 · admin 6 (getMaxSenders no
...
function _renderPlanosUI(d){
  ...
  box.innerHTML=['vip','vipro','doublepro'].map(pl=>{
    ...
    // "sem automático" quando o plano não inclui, ex.: VIP).
    ...
    // TEM que aparecer pro usuário — DoublePro 2 (revezamento), demais 1.
    ...
    if(d.ok) {
      ...
      if(d.duplicado) {
        ...
        const medTxt=(typeof med==="number"&&med>0)
          ...
          :'assim que possível';
        if(dMsg) dMsg.innerHTML = 'Recebemos seu pedido e o comprovante.<br>Um admin confirma manualmente — '+medTxt+' — e assim que confirmado, seu <strong>'+(NOME[plano]||plano)+'</strong> é ativado.<br><strong>Para dúvidas, entre em contato:</strong>';
