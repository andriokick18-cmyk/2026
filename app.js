...
function _vfEmpty(){return{q:"",estado:[],cidade:[],categoria:[],cargo:[],salarioMin:0,vagasMin:0,inicio:[],exp:[],temporada:[],visa:[],ocultarEncerradas:true,status:[],grupo:[],email:true,tipo:"all",ativa:false};}
...
function vfParams(ctx,st){
  ...
  for(const k of ["estado","cidade","categoria","cargo","status","grupo","temporada","visa"])(st[k]||[]).forEach(v=>p.append(k,v));
...
function _vfRenderSecs(d){
  ...
  // 💎 Double Pro: status no DOL e grupo da loteria — só quando a planilha tem o dado
  {
    const isDP=!!(d&&d.isDP);
    for(const k of ["status","grupo"]){
      const sel=st[k];
      if(!fac||live){show(k,false);continue;}
      const list=fac[k]||[];
      // v181: a mesma régua geral vale aqui (o status já tinha a dela desde o
      // v173; o grupo NÃO tinha e aparecia com 1 opção só). E, pra quem é DP,
      // a seção só aparece se houver opção DE VERDADE com os outros filtros
      // aplicados — em jul2026 (0 e-mail) o grupo abria VAZIO, sem nenhuma
      // opção clicável, porque nenhuma vaga passa no "só com e-mail".
