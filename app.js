...
function openAuthGate(step,intent){
  ...
  // pessoa só segue depois do "Li e entendi" (1x por sessão do navegador).
  // Qualquer erro aqui = fail-open (o aviso nunca pode impedir o login).
  try{
    if(window._avisoResetOn&&sessionStorage.getItem("h2bResetNoticeOk")!=="1"){
      const rm=g("#reset-notice-modal");
      if(rm){window._resetNoticePend=[step,intent];rm.style.display="flex";return;}
    }
  }catch(e){}
...
// 📢 v150: "Li e entendi" da janela do reset — marca a sessão e segue pro login
function resetNoticeOk(){
  try{sessionStorage.setItem("h2bResetNoticeOk","1");}catch(e){}
  const rm=g("#reset-notice-modal");if(rm)rm.style.display="none";
...
function renderPlanStatusCard(){
  ...
  const dmd=U.diamonds||{real:0,bonus:0};
  html+=`<div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-top:8px;font-size:12.5px">
    <span style="font-weight:800">${esc(t('mc_4'))}:</span>
    <span>💎 <strong>${(dmd.real||0).toLocaleString('pt-BR')}</strong> ${esc(t('mc_5'))}</span>
    ${(dmd.bonus||0)>0?`<span style="color:var(--t3)">🎁 ${(dmd.bonus||0).toLocaleString('pt-BR')} ${esc(t('mc_6'))}</span>`:''}
  </div>
  <div id="mc-pedido" style="font-size:11.5px;color:var(--t3);margin-top:6px"></div>
  </div>`;
  el.innerHTML=html;
  el.style.display="block";
  _mcUltimoPedido();
}
// 👤 2.0-P6: status da ÚLTIMA doação direto no card — a pessoa sabe na hora
// se está aguardando o admin, se os 💎 já caíram ou se foi cancelada.
async function _mcUltimoPedido(){
  ...
  try{
    ...
    // data nunca aparecia); e 'pago' NÃO é '💎 na conta' (o crédito só roda
    // no 'ativo') — status honesto próprio pra não mentir pro usuário.
...
function renderHdr(){
  ...
  if(plansBtn){
    ...
    if(U.plan&&U.plan!=="free"){
      ...
      g("#hdr-plans-label").textContent="💎 Diamantes";
...
function sv(v,...args){
  ...
  VIEWS.forEach(id=>{const ve=g("#v-"+id);if(ve)ve.classList.toggle("gone",id!==v);const si=g("#si-"+id);if(si)si.classList.toggle("active",id===v);const bn=g("#bn-"+id);if(bn)bn.classList.toggle("active",id===v);});
  if(v==="jobs"){setTimeout(loadLugares,400);}if(v==="plans"){setTimeout(()=>updatePlanCalc&&updatePlanCalc(),100);try{loadDiamonds();}catch(e){}}if(v==="profile"){loadProfile();loadTplView();setTimeout(()=>{_loadSoundPref();renderSoundSelector();},100);}
...
async function openModal(jobId){
  ...
  if(!j)return;
  if(U.manualRemaining<=0){sv("plans");toast("Limite diário atingido. Troque diamantes para mais envios.","r");return;}
...
function _renderNotifToggle(){
  ...
  if(msg){
    ...
    // de ninguém — este alerta é só de diamantes/missões/novidades, nunca de
    ...
    if(_notifEnabled){msg.style.color="var(--green)";msg.textContent="🛫 Ativado! Você será avisado de diamantes, missões e novidades.";}
    else{msg.style.color="var(--t3)";msg.textContent="Toque para ativar alertas de diamantes, missões e novidades.";}
  }
}
...
//  local de diamantes/missões/novidades). Removido o subsistema de
...
const LANG_DICT = {
  pt: {
    ...
    "rn_t":"Atualização importante — leia antes de entrar","rn_p1":"Atualizamos o H2BApply: os 3 servidores viraram UM só — todas as contas foram trazidas pra cá. NADA foi apagado: seus envios, dias de VIP, diamantes e currículos estão aqui. Entre usando apenas 1 e-mail, de preferência o e-mail principal que já estava cadastrado na sua conta.","rn_warn":"⚠️ Sistema anti-duplicação: se forem criadas 2 contas com o mesmo nome no currículo — mesmo com e-mails diferentes — as duas contas podem ser BANIDAS PERMANENTEMENTE. Use somente 1 conta.","rn_ok":"✅ Li e entendi — continuar", // 📢 v150: janela do reset antes do login
    "mc_1":"Minha Conta","mc_13":"PIX recebido — 💎 a caminho","mc_14":"Troque seus 💎 por mais dias na aba Planos.","mc_15":"ATENÇÃO: expira em","lb_ajuste":"Ajuste de bônus","lb_credito":"Crédito","mp_doa":"Doação","mp_diam":"diamantes","mp_pend":"⏳ Em análise","mp_pago":"💰 PIX recebido — 💎 a caminho","mp_cred":"💎 creditados","mp_canc":"✖ Cancelada","mp_canc_sub":"Se achar que foi engano, chame no WhatsApp","mp_motivo":"Motivo","mp_ileg":"⚠️ Não conseguimos ler seu comprovante — reenvie uma foto nítida.","mp_ok":"✅ Comprovante conferido pelo robô","mp_analise":"🔍 Comprovante em análise","mp_reenviar":"📤 Reenviar comprovante","mp_media":"confirmação média real","mp_sub24":"Analisamos e confirmamos — você recebe notificação","mp_vertodos":"ver todos", "mc_2":"Plano Grátis","mc_3":"envios manuais por dia","mc_4":"💎 Meus diamantes","mc_5":"reais","mc_6":"bônus (só valem pra troca de plano)","mc_7":"🧾 Última doação:","mc_8":"aguardando confirmação do admin","mc_9":"confirmada — 💎 na sua conta","mc_10":"cancelada","mc_11":"Você ainda não fez nenhuma doação.","mc_12":"Troque seus 💎 por um plano quando quiser — a troca é imediata.", // 👤 2.0-P6: Minha Conta em 5s
    "tut_t":"Central de Tutoriais","tut_s":"28 passo a passos com fotos reais — tudo o que dá pra fazer no H2BApply, explicado tela por tela","tut_ph":"🔍 O que você quer aprender? Ex.: currículo, automático, diamantes...","tut_v":"Nada encontrado com essa palavra — tente outro termo (ex.: \"enviar\", \"plano\", \"perfil\").","tut_load":"⏳ Carregando os tutoriais...", // 📖 v160
    ...
    "extra_gmail":"Gmail Extra para Envios","upgrade_to_enable":"Troque diamantes para ativar.","tap_to_pick":"(toque para escolher)","search_btn":"Buscar",
    ...
    "plans_send_title":"💎 Diamantes","plans_send_sub":"Turbine seus envios com diamantes", // 🌐 v166: 3º CTA da Home
    ...
    "enable_notif":"Ativar notificações","enable_notif_sub":"Receba avisos de diamantes, missões e novidades — mesmo com o app fechado",
    "activate":"Ativar","inbox_search_ph":"Buscar empresa, assunto...","notif_short":"Notif",
    "reply_alert":"Alerta de nova resposta","reply_alert_sub":"Som + aviso de diamantes, missões e novidades (o site não lê e-mail — não avisamos quando a empresa responde)",
  ...
  en: {
    ...
    "rn_t":"Important update — read before signing in","rn_p1":"We updated H2BApply: the 3 servers became ONE — every account was brought here. NOTHING was deleted: your sends, VIP days, diamonds and resumes are all here. Sign in using only 1 e-mail, preferably the main e-mail already registered on your account.","rn_warn":"⚠️ Anti-duplication system: if 2 accounts are created with the same name on the resume — even with different e-mails — both accounts can be PERMANENTLY BANNED. Use only 1 account.","rn_ok":"✅ I read and understood — continue", // 📢 v150: janela do reset antes do login
    "mc_1":"My Account","mc_13":"PIX received — 💎 on the way","mc_14":"Swap your 💎 for more days in the Plans tab.","mc_15":"ATTENTION: expires in","lb_ajuste":"Bonus adjustment","lb_credito":"Credit","mp_doa":"Donation","mp_diam":"diamonds","mp_pend":"⏳ Under review","mp_pago":"💰 PIX received — 💎 on the way","mp_cred":"💎 credited","mp_canc":"✖ Cancelled","mp_canc_sub":"If you think it was a mistake, message us on WhatsApp","mp_motivo":"Reason","mp_ileg":"⚠️ We could not read your receipt — resend a clear photo.","mp_ok":"✅ Receipt verified by the robot","mp_analise":"🔍 Receipt under review","mp_reenviar":"📤 Resend receipt","mp_media":"real average confirmation","mp_sub24":"We review and confirm — you will get a notification","mp_vertodos":"see all", "mc_2":"Free Plan","mc_3":"manual sends per day","mc_4":"💎 My diamonds","mc_5":"real","mc_6":"bonus (plan swaps only)","mc_7":"🧾 Last donation:","mc_8":"awaiting admin confirmation","mc_9":"confirmed — 💎 credited","mc_10":"cancelled","mc_11":"You haven't made any donation yet.","mc_12":"Swap your 💎 for a plan anytime — instantly.", // 👤 2.0-P6
    "tut_t":"Tutorials Center","tut_s":"28 step-by-step guides with real screenshots — everything you can do on H2BApply, screen by screen","tut_ph":"🔍 What do you want to learn? E.g.: resume, automatic, diamonds...","tut_v":"Nothing found — try another word (e.g. \"send\", \"plan\", \"profile\").","tut_load":"⏳ Loading tutorials...", // 📖 v160
    ...
    "extra_gmail":"Extra Gmail for Sending","upgrade_to_enable":"Exchange diamonds to enable.","tap_to_pick":"(tap to choose)","search_btn":"Search",
    ...
    "plans_send_title":"💎 Diamonds","plans_send_sub":"Power up your applications with diamonds", // 🌐 v166: Home 3rd CTA
    ...
    "enable_notif":"Enable notifications","enable_notif_sub":"Get alerts for diamonds, missions and news — even with the app closed",
    "activate":"Enable","inbox_search_ph":"Search company, subject...","notif_short":"Notif",
    "reply_alert":"New reply alert","reply_alert_sub":"Sound + alert for diamonds, missions and news (the site never reads your inbox — we can't tell you when a company replies)",
  ...
  es: {
    ...
    "rn_t":"Actualización importante — lee antes de entrar","rn_p1":"Actualizamos H2BApply: los 3 servidores se volvieron UNO solo — todas las cuentas fueron traídas aquí. NADA fue borrado: tus envíos, días de VIP, diamantes y currículums están aquí. Entra usando solo 1 e-mail, de preferencia el e-mail principal que ya estaba registrado en tu cuenta.","rn_warn":"⚠️ Sistema anti-duplicación: si se crean 2 cuentas con el mismo nombre en el currículum — incluso con e-mails diferentes — las dos cuentas pueden ser BANEADAS PERMANENTEMENTE. Usa solamente 1 cuenta.","rn_ok":"✅ Leí y entendí — continuar", // 📢 v150: janela do reset antes do login
    "mc_1":"Mi Cuenta","mc_13":"PIX recibido — 💎 en camino","mc_14":"Cambia tus 💎 por más días en la pestaña Planes.","mc_15":"ATENCIÓN: expira en","lb_ajuste":"Ajuste de bono","lb_credito":"Crédito","mp_doa":"Donación","mp_diam":"diamantes","mp_pend":"⏳ En revisión","mp_pago":"💰 PIX recibido — 💎 en camino","mp_cred":"💎 acreditados","mp_canc":"✖ Cancelada","mp_canc_sub":"Si crees que fue un error, escríbenos por WhatsApp","mp_motivo":"Motivo","mp_ileg":"⚠️ No pudimos leer tu comprobante — reenvía una foto nítida.","mp_ok":"✅ Comprobante verificado por el robot","mp_analise":"🔍 Comprobante en revisión","mp_reenviar":"📤 Reenviar comprobante","mp_media":"confirmación media real","mp_sub24":"Revisamos y confirmamos — recibirás una notificación","mp_vertodos":"ver todos", "mc_2":"Plan Gratis","mc_3":"envíos manuales por día","mc_4":"💎 Mis diamantes","mc_5":"reales","mc_6":"bono (solo para cambiar de plan)","mc_7":"🧾 Última donación:","mc_8":"esperando confirmación del admin","mc_9":"confirmada — 💎 en tu cuenta","mc_10":"cancelada","mc_11":"Aún no hiciste ninguna donación.","mc_12":"Cambia tus 💎 por un plan cuando quieras — al instante.", // 👤 2.0-P6
    "tut_t":"Central de Tutoriales","tut_s":"28 guías paso a paso con fotos reales — todo lo que puedes hacer en H2BApply, pantalla por pantalla","tut_ph":"🔍 ¿Qué quieres aprender? Ej.: currículum, automático, diamantes...","tut_v":"Nada encontrado — prueba otra palabra (ej. \"enviar\", \"plan\", \"perfil\").","tut_load":"⏳ Cargando los tutoriales...", // 📖 v160
    ...
    "extra_gmail":"Gmail Extra para Envíos","upgrade_to_enable":"Cambia diamantes para activar.","tap_to_pick":"(toca para elegir)","search_btn":"Buscar",
    ...
    "plans_send_title":"💎 Diamantes","plans_send_sub":"Potencia tus postulaciones con diamantes", // 🌐 v166: 3er CTA de Home
    ...
    "enable_notif":"Activar notificaciones","enable_notif_sub":"Recibe avisos de diamantes, misiones y novedades — incluso con la app cerrada",
    "activate":"Activar","inbox_search_ph":"Buscar empresa, asunto...","notif_short":"Notif",
    "reply_alert":"Alerta de nueva respuesta","reply_alert_sub":"Sonido + aviso de diamantes, misiones y novedades (el sitio no lee tu correo — no avisamos cuando la empresa responde)",
...
function updatePlanCalc() {
  ...
  // 💼 MC5-P2 item 7: a marcação dos packs usa a QUANTIDADE REAL da tabela
  // oficial (window._diamPacksById, preenchida pelo /api/diamonds) — os
  // números do HTML são só fallback pro app abrir sem rede.
  [67,100,167].forEach(id=>{const el=g('#dpack-'+id);if(!el)return;const q2=(window._diamPacksById||{})[id]||id;el.className='plan-option'+(q2===qty?' selected':'');});
  const price=(U.diamondPrice||1.5);
  ...
  if(cDesc)cDesc.textContent=qty+' \u{1F48E} × R$ '+price.toFixed(2).replace('.',',')+' · doação única';
...
async function loadDiamonds(){
  try{
    ...
    window._diamMediana=d.medianaAprovacaoHoras; // 💼 MC5-P2: promessa honesta
    _renderPacksFromTable(d); // 💼 MC5-P2 item 7 (13c): packs sempre da tabela
    renderDiamUpgrade(d);renderDiamTroca(d);renderDiamExtrato(d);
  }catch(e){
    // 💼 MC5-P2 item 9: falha nunca mais some em silêncio — card de retry
    const sc=g('#diam-saldo-card');
    if(sc)sc.innerHTML=`<div style="background:var(--surface);border:1.5px solid var(--border);border-radius:var(--rl);padding:14px;text-align:center;font-size:12px;color:var(--red)">Não deu pra carregar seus \u{1F48E}. <span style="text-decoration:underline;cursor:pointer;font-weight:700" onclick="loadDiamonds()">Tentar de novo</span></div>`;
  }
  loadMissoes(); // 🎁 v68: recompensas (retroativo — quem já cumpriu recebe na hora)
}
// 💼 MC5-P2 item 7 (regra 13c): os packs derivam SEMPRE da tabela oficial
// (/api/diamonds → planos) — o HTML estático é só esqueleto/fallback. Se um
// preço mudar (PLANO_PRECO_TAB ou DIAMOND_PRICE_BRL), os packs acompanham
// sozinhos — nunca mais induzem doação com valor errado (classe do bug 13f).
function _renderPacksFromTable(d){
  try{
    const packs=(d.planos||[]).filter(p2=>p2&&p2.dias===30&&["vip","vipro","doublepro"].includes(p2.plano));
    if(packs.length!==3)return;
    const map={vip:67,vipro:100,doublepro:167};
    const byId={};
    for(const p2 of packs){
      const id=map[p2.plano];byId[id]=p2.diamantes;
      const el=g('#dpack-'+id);if(!el)continue;
      el.setAttribute('onclick','setDiamQty('+p2.diamantes+')');
      const big=el.querySelector('div');
      if(big)big.innerHTML=big.innerHTML.replace(/^\d+\s/,p2.diamantes+' ');
    }
    window._diamPacksById=byId;
  }catch(e){}
}
...
function renderDiamExtrato(d){
  ...
  // 💼 MC5-P2 item 8: rótulos que faltavam (ajuste/crédito apareciam como
  // chave crua), "ver todos" (antes só 12 dos 60 lançamentos) e o SALDO
  // depois de cada lançamento (o servidor já mandava, ninguém mostrava).
  const LBL={doacao:'\u{1F49A} '+t('lb_doacao'),troca:'\u{1F501} '+t('lb_troca'),transfer_in:'\u{1F381} '+t('lb_tin'),transfer_out:'\u{1F381} '+t('lb_tout'),admin:'\u{1F6E0} '+t('lb_admin'),estorno:'\u26D4 '+t('lb_estorno'),missao:'\u{1F381} '+t('lb_missao'),correcao:'\u270F\uFE0F '+t('lb_correcao'),upgrade:'\u2B06\uFE0F '+t('lb_upgrade'),ajuste:'\u2696\uFE0F '+t('lb_ajuste'),credito:'\u2795 '+t('lb_credito')};
  const lim=window._diamExtratoAll?led.length:12;
  box.innerHTML=`<div style="background:var(--surface);border:1.5px solid var(--border);border-radius:var(--rl);overflow:hidden">
    ...
    ${led.slice(0,lim).map(e=>`<div style="display:flex;justify-content:space-between;gap:8px;padding:8px 16px;border-bottom:1px solid var(--border);font-size:12px">
      ...
      <div style="flex-shrink:0;text-align:right"><div style="font-weight:800;color:${(e.qtd||0)>=0?'var(--green)':'var(--red)'}">${(e.qtd||0)>=0?'+':''}${e.qtd||0} \u{1F48E}</div>${(e.saldoReal!=null)?`<div style="font-size:9.5px;color:var(--t3)">saldo: ${(e.saldoReal||0)+(e.saldoBonus||0)}</div>`:''}</div>
    </div>`).join('')}
    ${led.length>lim?`<div style="padding:8px 16px;text-align:center;font-size:11.5px;color:var(--blue);cursor:pointer;font-weight:700" onclick="window._diamExtratoAll=true;renderDiamExtrato(_diamData)">${esc(t('mp_vertodos'))} (${led.length})</div>`:''}
  </div>`;
}
...
// cru como sempre foi (fail-open, nunca trava o envio da doação).
...
async function loadMeusPagamentos(){
  ...
  try{
    ...
    _mpLoaded=true;
    // 💼 MC5-P2: renderer da ERA DOAÇÃO (o antigo era da era plano e mostrava
    // "doacao · ?d"): status em vocabulário de doação, mediana REAL de
    // confirmação no lugar do "24h" fixo, motivo do cancelamento visível e
    // status do comprovante (com reenvio em 1 toque quando ilegível).
    const _med=window._diamMediana;
    ...
    const stLbl=(p)=>{
      const ehDoa=p.tipo==="doacao"||p.plano==="doacao";
      ...
      if(p.status==="ativo")return[ehDoa?t('mp_cred'):"✅ Ativo","#059669","rgba(16,185,129,.12)",p.ativadoEm?("✔ "+new Date(p.ativadoEm).toLocaleDateString("pt-BR")):""];
    ...
    box.innerHTML=codeNote+peds.map(p=>{
      ...
      const ehDoa=p.tipo==="doacao"||p.plano==="doacao";
      const planLbl=ehDoa
        ?("💎 "+t('mp_doa')+" · "+(parseInt(p.diamantesCreditados||p.diamantes,10)||Math.round((+p.valorTotal||0)/(U.diamondPrice||1.5)))+" "+t('mp_diam'))
