...
let CFG={name:"",country:"Brazil",phone:"",city:"",subject:"",body:""};
...
let HIST=[],APPLIED=new Set();

// ── Sistema de filtro de vagas ──
var _sentJan=new Set(),_sentJul=new Set(),_sentAll=new Set(),_sentLoaded=false;
...
// ── 🔁 v185 LOTE 3: applyStatus(d) — FONTE ÚNICA do "o que o servidor diz
// sobre esta conta". Antes só o checkStatus() do boot montava o U inteiro; o
// syncData() (chamado depois de comprar, de conectar Gmail, de voltar pra
// Home) atualizava só manualLimit/autoLimit/remaining. Resultado real: a
// ATIVAÇÃO PROVISÓRIA (intencional desde 21/07 — comprovante que confere
// libera o plano NA HORA) existia no servidor e era INVISÍVEL na tela: a
// pessoa clicava em "Ir para Envio Automático" logo após comprar e batia no
// cartão vermelho "Plano necessário", porque U.needsPlan/U.vip continuavam
// os do boot. Só um F5 resolvia, e ninguém sabia disso.
// ⚠️ Esta função NÃO mexe em tela nenhuma (nada de showApp/sv) — senão
// chamá-la no meio do checkout jogaria a pessoa pra Home.
function applyStatus(d){
  ...
  if(_antes&&(_antes.needsPlan!==!!U.needsPlan||_antes.plan!==U.plan||_antes.gmail!==!!U.gmailConnected))_pendingOrderCache=undefined;
}
...
async function checkStatus(){
  ...
  try{
    if(d.connected){
      ...
      // v185 LOTE 3: `estado` passou a viajar no /api/status (o cadastro v175 já
      // obriga o campo) — é o que deixa o passo 2 da compra confirmar os dados
      // em vez de pedir tudo de novo.
      CFG={name:d.name||"",country:d.country||"Brazil",phone:d.phone||"",whatsapp:d.whatsapp||"",city:d.city||"",estado:d.estado||"",subject:d.settings?.subject||"",body:d.settings?.body||""};
      DOCS=d.cvs||[];
...
// 🐛 v225 (achado do dono, teste ao vivo logo depois de um deploy): durante
// o restart do servidor no Render, a API pode responder com a página de
// erro HTML do proxy em vez de JSON — `r.json()` cru lançava "Unexpected
// token '<', '<!DOCTYPE'... is not valid JSON" e esse erro técnico
// aparecia direto pro usuário. Função ÚNICA usada por TODA leitura de
// resposta do portão de autenticação (login/cadastro/recuperação de
// senha): distingue "a resposta nem é JSON de verdade" (deploy/restart —
// mensagem amigável e honesta) de "JSON de erro normal do servidor" (ex.:
// senha errada, código inválido — segue pro `d.error` de sempre).
async function _agReadJson(r){
  try{ return await r.json(); }
  catch(e){ throw new Error("O servidor está reiniciando (isso acontece logo depois de um deploy) — espere alguns segundos e tente de novo."); }
}
...
// "🤖 Turbo"/"⭐Pro"/"Turbo"...). PLAN_NAMES só vale onde o rótulo se refere
...
// v218 (reestruturação de Planos, 20/09/2026): nomes públicos novos — a
// CHAVE interna (vip/vipro/doublepro) não mudou, só o que a pessoa lê.
const PLAN_NAMES={vip:"✋ Manual",vipro:"⚡ Turbo",doublepro:"🚀 Máximo"};
...
// só tem o automático, então o header mostrava o nome errado pra uma conta
// com 0 manuais/dia. Aqui, quem só tem o manual é "✋ Manual", quem só tem o
// robô é "🤖 Automático", quem tem os dois é "⚡ Turbo" (ou "🚀 Máximo", se
// for o caso) e quem não tem nada é "Grátis". v218: nomes atualizados pra
// Manual/Turbo/Máximo — PLAN_NAMES é a fonte única dos 3.
function planLabelAtivo(){
  ...
  if(m&&a)return String(U.plan||"")==="doublepro"?PLAN_NAMES.doublepro:"✋⚡ Turbo";
  return m?PLAN_NAMES.vip:"🤖 Pro";
}
...
function planBadgeHTML(){
  ...
  let html="";
  if(hasManual)html+=`<span class="tag" style="background:var(--bluel);color:${col(ml)};border-color:${col(ml)};font-size:10px;white-space:nowrap">✋ Manual ${ml} dia${ml===1?"":"s"}</span> `;
...
function renderPlanStatusCard(){
  ...
  if(hasManual)html+=row("✋","Manual",ml,v.manualExpires);
  if(hasAuto)html+=row("🤖","Automático",al,v.autoExpires);
  if(!hasManual&&!hasAuto&&U.isAdmin){
    ...
    // escondia o zero e MENTIA um limite grátis que não existe mais) é
    // confuso — a mensagem certa é "sem plano ativo" + CTA direto pra Planos.
    html+=`<div style="background:var(--redl);border:1.5px solid var(--redb);border-radius:var(--r);padding:12px 14px;margin-bottom:8px">
      <div style="font-weight:800;font-size:13px;color:var(--red)">🔒 ${esc(t('mc_2'))}</div>
      <div style="font-size:11px;color:var(--t2);margin-top:3px">${esc(t('mc_12'))}</div>
      <button class="btn btn-primary btn-sm w100" style="margin-top:10px;justify-content:center" onclick="sv('plans')">${esc(t('ns4_c'))} <i class="ti ti-arrow-right"></i></button>
    </div>`;
  }
...
function setTab(t){
  tab=t;
  // v173: filtros por planilha (VF) — carrega o que a pessoa tinha nesta aba
  vfLoad("manual");vfRenderChips("manual");
  Promise.resolve(_ensureCatLabels()).then(()=>vfRenderChips("manual"));
  setTimeout(loadLugares,250); // v114: recarrega sugestões da planilha nova
  // v22: planilhas suportam ordenação real (random/desc/wage/start via modal de filtros)
  if(!["random","desc","wage","start","match"].includes(fSort)){setSort("random");}
...
// do automático, então quem nunca abria o automático nunca
...
function _ensureCatLabels(){
  ...
  _catLabelsLoading=fetch("/api/category-groups",{credentials:"include"})
    .then(r=>r.json())
    .then(d=>{window._catLabels=d.labels||{};})
    .catch(()=>{window._catLabels=window._catLabels||{};});
  return _catLabelsLoading;
}


// ═══════════════════════════════════════════════════════════════════════
//  🔍 v173 — FILTROS DE VAGAS: SISTEMA NOVO, ÚNICO (Envio Manual + Automático)
//  Ordem do dono (13/09/2026, reafirmada): apagar o sistema antigo e criar um
//  novo pesquisando o mercado (site oficial do DOL, Indeed/LinkedIn, boards
//  sazonais, Baymard/NN-g). Regras inegociáveis que este bloco cumpre:
//  • um filtro NUNCA elimina o outro: E entre dimensões, OU dentro da mesma
//    (Flórida OU Texas) — marcar uma 2ª opção do mesmo grupo sempre SOMA;
//  • contagem AO VIVO em 2 lugares: ao lado de CADA opção (calculada no
//    servidor com todos os OUTROS filtros aplicados — /api/vagas/filtros) e
//    no botão fixo "Ver N vagas"; a contagem é a verdade pós-filtro (já sem
//    as vagas enviadas/na fila — regra 8);
//  • opção que daria 0 fica cinza (não clicável); dimensão que a planilha
//    não tem (sem salário/data/e-mail publicado) some com aviso honesto;
//  • mobile-first: bottom sheet com rodapé fixo, chips removíveis fora do
//    painel, "Limpar tudo", alvos ≥44px, tudo via LANG_DICT (regra 6f).
//  O MESMO estado/painel serve os 2 contextos ("manual" = lista de vagas,
//  "auto" = passo 2 do robô); o objeto de filtros do automático viaja no
//  /api/auto/start e o servidor usa o MESMO motor no refill (regra 13).
// ═══════════════════════════════════════════════════════════════════════
// ⏳ v181 LOTE 6 (decisão do dono): o estado PADRÃO esconde a vaga cuja
// temporada JÁ TERMINOU (220 na H-2A hoje) — a pessoa gastava envio e limite
// diário com vaga que não existe mais. Não é um filtro escondido: um chip
// permanente sobre a lista diz quantas estão escondidas e mostra todas em 1
...
// v181: rótulos das dimensões novas — sempre pelo dicionário (regra 6f)
function _vfExpLabel(v){return v===0?t('vf_exp_0'):t('vf_exp_ate').replace("{n}",v);}
function _vfTempLabel(v){return v==="futura"?t('vf_temp_futura'):v==="aberta"?t('vf_temp_aberta'):v==="encerrada"?t('vf_temp_encerrada'):t('vf_temp_semdata');}
// "2026-03" → "Mar/26". Aceita também o formato antigo (número 1-12), que
// ainda pode estar salvo no aparelho ou num job.filters de robô rodando.
function _vfMesLabel(x){
  const M=_vfMeses();
  if(x&&typeof x==="object"&&x.mes)return M[x.mes-1]+"/"+String(x.ano).slice(2);
  const s=String((x&&x.v!==undefined?x.v:x)||"");
  const am=s.match(/^(\d{4})-(\d{1,2})$/);
  if(am)return M[parseInt(am[2],10)-1]+"/"+am[1].slice(2);
  const m=parseInt(s,10);
  return (m>=1&&m<=12)?M[m-1]:s;
}

function _vfSheet(ctx){return ctx==="auto"?(autoSelectedSrc||""):tab;}
...
function _vfSyncQInput(){const el=g("#q");const q=(VF.st.manual&&VF.st.manual.q)||"";if(el&&el.value!==q)el.value=q;_vfSyncQClear();}
// 🔎 v182 LOTE 9: o × do campo de busca (o campo irmão da aba Enviadas já
// tinha) — a pessoa filtrava, saía da tela, voltava e não tinha onde clicar
// pra desfazer. Ele aparece só quando há texto, e limpar é UM clique.
function _vfSyncQClear(){const b=g("#q-clear");if(b)b.style.display=(g("#q")&&g("#q").value)?"flex":"none";}
function limparBusca(){const el=g("#q");if(el)el.value="";_vfSetQ("");qSugClose();_vfSyncQClear();vfAfterChange("manual");el&&el.focus();}
// A fileira de filtros/ordenação rola na horizontal: a borda apagada só liga
// enquanto ainda há botão fora da tela (nunca mente dizendo que tem mais).
function _vfSyncScrollHint(){
  const r=g("#filters-row");if(!r)return;
  r.classList.toggle("has-more",r.scrollWidth-r.clientWidth-r.scrollLeft>6);
}
...
function vfLoad(ctx){
  let st=null;try{const raw=localStorage.getItem(_vfKey(ctx));st=raw?JSON.parse(raw):null;}catch(e){st=null;}
...
function vfParams(ctx,st){
  st=st||VF.st[ctx];const p=new URLSearchParams();
  for(const k of ["estado","cidade","categoria","cargo","grupo","temporada","visa"])(st[k]||[]).forEach(v=>p.append(k,v));
  (st.inicio||[]).forEach(m=>p.append("inicio",String(m)));
  (st.exp||[]).forEach(v=>p.append("exp",String(v)));
  // padrão do app (esconder temporada encerrada) — declarado por chip próprio
  if(st.ocultarEncerradas&&!(st.temporada||[]).length)p.set("ocultarEncerradas","1");
...
function vfOpen(ctx){
  ...
  if(ctx==="auto"&&!autoSelectedSrc){toast(t('vf_pick_source'),"r");return;}
  const ov=g("#vf-overlay");if(!ov)return;
  VF.ctx=ctx;VF.draft=_vfClone(VF.st[ctx]);VF.busca={estado:"",cidade:"",cargo:""};VF.mais={};VF.facDraft=null;
  g("#vf-title").textContent=ctx==="auto"?t('vf_title_auto'):t('vf_title');
  ov.classList.remove("gone");
  _vfBuildSecs();
  vfRefresh(true);
  // ♿ v182 LOTE 9: o painel tinha role=dialog mas o foco continuava no body —
  // o Tab passeava pela página de trás e o Escape não fechava nada. Agora o
  // foco entra no painel ao abrir e VOLTA pro botão que o abriu ao fechar.
  VF.foco=document.activeElement;
  setTimeout(()=>{const alvo=ov.querySelector("#vf-body input,#vf-body button,#vf-clear");if(alvo&&typeof alvo.focus==="function")alvo.focus();},60);
}
function vfClose(){
  const ov=g("#vf-overlay");if(ov)ov.classList.add("gone");
  VF.draft=null;
...
function vfApply(){
  ...
  vfClose();vfAfterChange(ctx);
}
// Toda mudança no painel → estado do rascunho + contagens novas (debounce
// curto; nunca recarrega a lista por trás — "frozen filters" é proibido).
function vfRefresh(imediato){
  clearTimeout(VF.timer);
  VF.timer=setTimeout(async()=>{
    _vfRenderSecs(VF.facDraft); // re-pinta a seleção na hora (otimista)
...
const VF_SECS=[
  ...
  {k:"categoria",ico:"📂"},
  {k:"exp",ico:"🧰"},        // v181: "não exige experiência" é a 1ª pergunta de quem nunca foi
  {k:"estado",ico:"📍",busca:"estado"},
  {k:"salario",ico:"💰"},
  {k:"temporada",ico:"🗓️"},  // v181: "ainda vai começar / já começou / encerrada"
  {k:"inicio",ico:"📅"},
  {k:"visa",ico:"🛂"},       // v181: só aparece em planilha com os 2 vistos misturados

  {k:"cidade",ico:"🏙️",busca:"cidade"},
  {k:"cargo",ico:"🏷️",busca:"cargo"},
  {k:"vagas",ico:"👥"},
  {k:"email",ico:"✉️",manual:true},
  {k:"grupo",ico:"🎟️",dp:true},
];
function _vfBuildSecs(){
  const body=g("#vf-body");if(!body)return;
  const secs=VF_SECS.filter(s=>{
    if(s.auto&&VF.ctx!=="auto")return false;
    if(s.manual&&VF.ctx!=="manual")return false;
    return true;
  });
  VF.secs=secs;
...
// o foco pro botão) e a fileira de filtros avisa que ainda tem botão fora da
...
function vfBusca(dim,v){
  VF.busca[dim]=String(v||"").trim();
  if(dim==="estado"){_vfRenderSecs(VF.facDraft);return;} // lista de estados já veio inteira — filtra local
  vfRefresh(); // cidade/cargo: busca no servidor (listas longas)
}
function vfMais(k){VF.mais[k]=!VF.mais[k];_vfRenderSecs(VF.facDraft);}
// Alterna uma opção (multi) / define valor (limiar) no RASCUNHO
function vfSet(dim,v){
  const d=VF.draft;if(!d)return;
  if(dim==="salarioMin"||dim==="vagasMin"){d[dim]=(d[dim]===+v)?0:+v;}
  else if(dim==="email"||dim==="ativa"){d[dim]=!d[dim];}
  else if(dim==="tipo"){d.tipo=v;}
  else if(dim==="inicio"){const i=d.inicio.indexOf(v);if(i>=0)d.inicio.splice(i,1);else d.inicio.push(v);d.inicio.sort();}
  else if(dim==="exp"){const m=+v;const arr=d.exp||(d.exp=[]);const i=arr.indexOf(m);if(i>=0)arr.splice(i,1);else arr.push(m);arr.sort((a,b)=>a-b);}

  else{const arr=d[dim]||(d[dim]=[]);const i=arr.indexOf(v);if(i>=0)arr.splice(i,1);else{ arr.push(v);} }
  vfRefresh();
}
const _vfAttr=v=>JSON.stringify(String(v)).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;");
const _vfOpt=(dim,v,label,n,on,disp)=>{
  const off=(n===0&&!on);
  const cnt=(typeof n==="number")?`<span class="n">${n.toLocaleString("pt-BR")}</span>`:"";
  // v182 LOTE 9: `aria-pressed` — a seleção era comunicada SÓ por cor, então
  // quem usa leitor de tela não tinha como saber o que já está marcado.
  return `<button type="button" class="vf-opt${on?" on":""}${off?" off":""}" aria-pressed="${on?"true":"false"}" ${off?'disabled aria-disabled="true"':""} onclick="vfSet('${dim}',${_vfAttr(v)})">${esc(label)}${cnt}</button>`;
};
function _vfRenderSecs(d){
  ...
  const show=(k,vis)=>{const el=g("#vf-sec-"+k);if(el)el.style.display=vis?"":"none";};
  const opts=(k,html)=>{const el=g("#vf-opts-"+k);if(el)el.innerHTML=html;};
  const maisBtn=(k,resto)=>resto>0?`<button type="button" class="vf-more" onclick="vfMais('${k}')">${VF.mais[k]?esc(t('vf_less')):esc(t('vf_more')).replace("{n}",resto)}</button>`:"";
  const lim=(k,arr,base)=>VF.mais[k]?arr:arr.slice(0,base);
  // 🙈 v181 LOTE 6 — REGRA ÚNICA: dimensão com menos de 2 valores distintos
  // não separa NADA (marcar a única opção não muda uma vaga sequer). Antes só
  // o status tinha essa proteção, e as outras sumiam caladas quando a planilha
  // não tinha o dado. Agora a régua é a mesma pra todas — e a seção explica o
  // motivo em vez de desaparecer sem dizer nada.
  // ⏳ v182 LOTE 9 — DIMENSÃO SEM DADO FALA, NÃO SOME. Salário e e-mail já
  // avisavam em âmbar ("o governo ainda não publicou") desde o v173; cidade,
  // mês de início, cargo, vagas abertas, experiência e temporada simplesmente
  // DESAPARECIAM. Em jan2026/jul2025 isso apagava seções inteiras do painel e
  // quem vinha da H-2A concluía que o filtro quebrou. Mesmo aviso pra todas.
  const _semDado=(k,chave)=>{show(k,true);setN(k,"");opts(k,`<div class="vf-note vf-note-warn">⏳ ${esc(t(chave))}</div>`);};
  const _soUmValor=(k,sel,rotulo)=>{
    const dn=disp?disp[k+"Distintos"]:undefined;
    // v182 LOTE 9 (achado na revisão real no Chromium): com ZERO valor distinto
    // isto escondia a seção CALADA — e era justamente o caso de "a planilha não
    // tem esse dado", que agora tem aviso próprio. Só 1 valor distinto continua
    // sendo assunto desta função.
    if(dn===undefined||dn===0||dn>=2||(sel&&sel.length))return false;
    if(dn===1&&rotulo){show(k,true);setN(k,"");
      opts(k,`<div class="vf-note">${esc(t('vf_um_valor').replace("{v}",rotulo))}</div>`);}
    else show(k,false);
    return true;
  };

  // 🔎 palavra-chave (só automático — no manual a busca mora na barra)
  setN("q",st.q?"1":"");
  // 📂 tipo de trabalho
  {
    const list=fac?fac.categoria:[];
    const sel=st.categoria;
    if(!fac||!_soUmValor("categoria",sel,(list[0]&&(list[0].label||list[0].v))||"")){
      const ordered=[...list.filter(x=>sel.includes(x.v)),...list.filter(x=>!sel.includes(x.v))];
      const vis=lim("categoria",ordered,8);
      opts("categoria",vis.map(x=>_vfOpt("categoria",x.v,x.label||x.v,x.n,sel.includes(x.v))).join("")+maisBtn("categoria",ordered.length-vis.length));
      setN("categoria",sel.length?String(sel.length):"");
      show("categoria",!disp||disp.categoria>0||sel.length);
    }
  }
  // 🧰 v181: experiência exigida (meses) — "não exige experiência" é o maior
  // filtro que os dados já sustentavam e o site não tinha.
  {
    const sel=st.exp||[];
    if(!fac){show("exp",false);}
    else if(disp.exp===0&&!sel.length){_semDado("exp",'vf_no_exp');} // planilha sem o dado
    else if(!_soUmValor("exp",sel,(fac.exp&&fac.exp.length?_vfExpLabel(fac.exp[0].v):""))){
      show("exp",true);
      const lst=(fac.exp||[]).filter(x=>x.n>0||sel.includes(x.v));
      opts("exp",lst.map(x=>_vfOpt("exp",x.v,_vfExpLabel(x.v),x.n,sel.includes(x.v))).join("")+
        (fac.expSemDado>0?`<div class="vf-note">${esc(t('vf_exp_sem').replace("{n}",Number(fac.expSemDado).toLocaleString("pt-BR")))}</div>`:""));
      setN("exp",sel.length?String(sel.length):"");
    }
  }
  // 🗓️ v181: temporada (ainda vai começar / já começou / encerrada)
  {
    const sel=st.temporada||[];
    if(!fac){show("temporada",false);}
    else if(disp.temporada===0&&!sel.length){_semDado("temporada",'vf_no_temporada');} // planilha sem data
    else if(!_soUmValor("temporada",sel,(fac.temporada||[]).filter(x=>x.n>0).map(x=>_vfTempLabel(x.v))[0]||"")){
      show("temporada",true);
      const lst=(fac.temporada||[]).filter(x=>x.n>0||sel.includes(x.v));
      opts("temporada",lst.map(x=>_vfOpt("temporada",x.v,_vfTempLabel(x.v),x.n,sel.includes(x.v))).join("")+
        (st.ocultarEncerradas&&!sel.length?`<div class="vf-note">${esc(t('vf_temp_nota'))}</div>`:""));
      setN("temporada",sel.length?String(sel.length):"");
    }
  }

  // 📍 estado
  {
    const sel=st.estado;const list=fac?fac.estado:[];
    if(!fac||!_soUmValor("estado",sel,list[0]?_vfEstadoNome(list[0].v):"")){
      show("estado",true);
      const q=VF.busca.estado.toLowerCase();
      const ordered=[...list.filter(x=>sel.includes(x.v)),...list.filter(x=>!sel.includes(x.v))].filter(x=>!q||x.v.toLowerCase().includes(q)||_vfEstadoNome(x.v).toLowerCase().includes(q));
      const vis=q?ordered:lim("estado",ordered,8);
      opts("estado",vis.map(x=>_vfOpt("estado",x.v,_vfEstadoNome(x.v),x.n,sel.includes(x.v))).join("")+(q?"":maisBtn("estado",ordered.length-vis.length)));
      setN("estado",sel.length?String(sel.length):"");
    }
  }
  // 🛂 v181: tipo de visto — só aparece quando a planilha tem os DOIS
  {
    const sel=st.visa||[];
    if(!fac){show("visa",false);}
    else if(!_soUmValor("visa",sel,(fac.visa&&fac.visa[0]&&fac.visa[0].v)||"")){
      show("visa",true);
      opts("visa",(fac.visa||[]).map(x=>_vfOpt("visa",x.v,(x.v==="H-2A"?"🌾 ":"🔧 ")+x.v,x.n,sel.includes(x.v))).join(""));
      setN("visa",sel.length?String(sel.length):"");
    }
  }

  // 💰 salário mínimo por hora
  {
    const el=g("#vf-opts-salario");
    if(fac){
    ...
    } else if(el&&!el.innerHTML)opts("salario",`<span class="spin spin-sm"></span>`);
    setN("salario",st.salarioMin>0?"$"+st.salarioMin+"+":"");
  }
  // 📅 mês de início — v181: COM ANO ("Mar/26"). Sem o ano, "Mar 1.114" na
  // H-2A fazia a pessoa entender "vou começar em março" quando estava
  // filtrando contratos que COMEÇARAM em março. Os meses inteiramente no
  // passado ficam agrupados no fim (visíveis, nunca escondidos).
  {
    const sel=st.inicio||[];
    if(!fac){show("inicio",false);}
    else if(!(disp.inicio>0||sel.length)){_semDado("inicio",'vf_no_inicio');}
    else if(!_soUmValor("inicio",sel,fac.inicio&&fac.inicio[0]?_vfMesLabel(fac.inicio[0]):"")){
      show("inicio",true);
      const list=fac.inicio||[];
      const fut=list.filter(x=>!x.passado),pas=list.filter(x=>x.passado).reverse();
      const selecionados=list.filter(x=>sel.includes(x.v)&&x.passado);
      const vis=VF.mais.inicio?[...fut,...pas]:(fut.length?[...fut,...selecionados]:pas.slice(0,8));
      const resto=VF.mais.inicio?0:(fut.length?pas.length-selecionados.length:pas.length-vis.length);
      opts("inicio",vis.map(x=>_vfOpt("inicio",x.v,_vfMesLabel(x),x.n,sel.includes(x.v))).join("")+
        (resto>0?`<button type="button" class="vf-more" onclick="vfMais('inicio')">${esc(t('vf_meses_passados').replace("{n}",resto))}</button>`:"")+
        (VF.mais.inicio?`<button type="button" class="vf-more" onclick="vfMais('inicio')">${esc(t('vf_less'))}</button>`:""));
      setN("inicio",sel.length?String(sel.length):"");
    }
  }
  ...
  {
    const sel=st.cidade;
    if(fac&&!VF.busca.cidade&&_soUmValor("cidade",sel,(fac.cidade&&fac.cidade[0])?_vfCidadeLabel(fac.cidade[0].v,fac.cidade[0].label,fac.cidade[0].estado):"")){/* 1 cidade só: a seção explica */}
    else if(fac){
      const has=disp.cidade>0||sel.length;
      if(!has){_semDado("cidade",'vf_no_cidade');}else show("cidade",has);
      if(has){

        const regs=(fac.regiao||[]).filter(r=>r.n>0||sel.includes(r.v));
        const cities=fac.cidade||[];
        const ordered=[...cities.filter(x=>sel.includes(x.v)),...cities.filter(x=>!sel.includes(x.v))];
    ...
    } else show("cidade",false);
    setN("cidade",sel.length?String(sel.length):"");
  }
  // 🏷️ cargo específico
  {
    const sel=st.cargo;
    if(fac&&!VF.busca.cargo&&_soUmValor("cargo",sel,(fac.cargo&&fac.cargo[0])?(fac.cargo[0].label||fac.cargo[0].v):"")){/* 1 cargo só: a seção explica */}
    else if(fac){
      const has=disp.cargo>0||sel.length;
      if(!has){_semDado("cargo",'vf_no_cargo');}else show("cargo",has);
      if(has){

        const list=fac.cargo||[];
        const ordered=[...list.filter(x=>sel.includes(x.v)),...list.filter(x=>!sel.includes(x.v))];
        const vis=VF.busca.cargo?ordered:lim("cargo",ordered,8);
        opts("cargo",vis.map(x=>_vfOpt("cargo",x.v,x.label||x.v,x.n,sel.includes(x.v))).join("")+(VF.busca.cargo?"":maisBtn("cargo",ordered.length-vis.length))+(!vis.length?`<div class="vf-note">${esc(t('vf_nothing'))}</div>`:""));
      }
    } else show("cargo",false);
    setN("cargo",sel.length?String(sel.length):"");
  }
  // 👥 vagas abertas na empresa (trabalhadores solicitados)
  {
    if(fac){
      const has=disp.vagas>0||st.vagasMin>0;
      if(!has){_semDado("vagas",'vf_no_vagas');}else show("vagas",has);
      if(has)opts("vagas",fac.vagas.limiares.filter(l=>l.v>1&&(l.n>0||st.vagasMin===l.v)).map(l=>_vfOpt("vagasMin",l.v,t('vf_workers_more').replace("{v}",l.v),l.n,st.vagasMin===l.v)).join(""));
    } else show("vagas",false);
    setN("vagas",st.vagasMin>0?st.vagasMin+"+":"");
  }
  // ✉️ só com e-mail (manual)
  {
    if(VF.ctx==="manual"&&fac){
      const e=fac.email;
      const nota=disp.email===0?`<div class="vf-note vf-note-warn">⏳ ${esc(t('vf_no_email'))}</div>`:"";
      opts("email",`<label class="vf-toggle"><input type="checkbox" ${st.email?"checked":""} onchange="vfSet('email')"><span class="vf-sw"></span><span class="vf-toggle-lbl">${esc(t('vf_email_lbl'))}<small>${esc(t('vf_email_com').replace("{n}",e.com.toLocaleString("pt-BR")))} · ${esc(t('vf_email_sem').replace("{n}",e.sem.toLocaleString("pt-BR")))}</small></span></label>`+nota);
    }
    setN("email",st.email?"✓":"");
  }
  // 💎 Double Pro: grupo da loteria — só quando a planilha tem o dado
  // v209/v210: a seção "status no DOL" (Certified/Pending/Withdrawn/Denied/
  // Expired) foi REMOVIDA — regra 0.2 (nenhum status de vaga pro usuário,
  // "expirada" citada nominalmente) cobre isso tanto quanto o badge ativa/
  // inativa; era dado real do governo, mas a regra é sobre nunca comunicar
  // esse conceito, não sobre a veracidade do dado.
  {
    const isDP=!!(d&&d.isDP);
    for(const k of ["grupo"]){
      const sel=st[k];
      if(!fac){show(k,false);continue;}
      const list=fac[k]||[];
      // v181: pra quem é DP, a seção só aparece se houver opção DE VERDADE
      // com os outros filtros aplicados — em jul2026 (0 e-mail) o grupo
      // abria VAZIO, sem nenhuma opção clicável, porque nenhuma vaga passa
      // no "só com e-mail".
      const has=(((disp[k+"Distintos"]||0)>1)||sel.length)&&(!isDP||list.length>0);
      show(k,has);
      ...
      opts(k,list.map(x=>_vfOpt(k,x.v,t('vf_grupo_lbl').replace("{g}",x.v),x.n,sel.includes(x.v))).join("")+
        `<div class="vf-note">${esc(t('vf_grupo_expl'))}</div>`);
...
function _vfSyncSortBtns(){
  ...
  for(const id of ["so-start","so-desc"]){const b=g("#"+id);if(b)b.style.display=semData?"none":"";}
  if(semData&&(fSort==="start"||fSort==="desc"))setSort("random");
}
function _vfRenderFoot(d){
  ...
  if(!d){btn.innerHTML=`<span>${esc(t('vf_apply_simple'))}</span>`;btn.classList.remove("vf-zero-btn");return;}
  const n=d.total||0;
  const txt=(VF.ctx==="auto"?t('vf_apply_auto'):t('vf_apply')).replace("{n}",n.toLocaleString("pt-BR"));
  const de=d.totalBase>n?` <small>${esc(t('vf_of').replace("{m}",d.totalBase.toLocaleString("pt-BR")))}</small>`:"";
  btn.innerHTML=`<span>${esc(txt)}</span>${de}`;
  btn.classList.toggle("vf-zero-btn",n===0);
  const zero=g("#vf-zero");if(zero){zero.style.display=(n===0&&vfAtivos(VF.draft))?"":"none";zero.textContent=t('vf_zero');}
}
// ── Fora do painel: chips removíveis + badge + efeitos ───────────────────
function _vfChipList(ctx){return _vfChipsDe(VF.st[ctx],ctx);}
// v182 LOTE 9: os rótulos dos chips passaram a sair de um estado QUALQUER (não
// só do VF.st) — é assim que o modal do 📡 Radar consegue dizer, com as mesmas
// palavras da tela, exatamente o que está sendo avaliado.
function _vfChipsDe(st,ctx){
  const M=_vfMeses();const c=[];
  if(!st)return c;
  ...
  if((st.exp||[]).length)push("exp","","🧰 "+_vfExpLabel(Math.max.apply(null,st.exp)),"var(--green)");
  if((st.temporada||[]).length)push("temporada","","🗓️ "+st.temporada.map(v=>_vfTempLabel(v)).join(", "),"var(--blue)");
  if((st.visa||[]).length)push("visa","","🛂 "+st.visa.join(", "),"var(--blue)");
  ...
  if(ctx==="manual"&&st.email&&VF.fac.manual&&VF.fac.manual.facetas.email.sem>0)push("email","","✉️ "+t('vf_chip_email'),"var(--green)");
  ...
  if(ctx==="manual"&&!st.email)push("email","","✉️ "+t('vf_chip_sem_email'),"#d97706");

  // ⏳ v181 (decisão do dono): chip PERMANENTE e honesto do padrão que esconde
  // a temporada encerrada — com o número REAL vindo da faceta (que é calculada
  // sem o próprio filtro) e "mostrar" em 1 clique. Só aparece quando há de
  // fato vaga escondida; transparência total, nunca um corte silencioso.
  if(st.ocultarEncerradas&&!(st.temporada||[]).length){
    const fc=VF.fac[ctx];
    const enc=fc&&fc.facetas&&fc.facetas.temporada?(fc.facetas.temporada.find(x=>x.v==="encerrada")||{}).n||0:0;
    if(enc>0)push("ocultarEncerradas","","⏳ "+t('vf_chip_encerradas').replace("{n}",Number(enc).toLocaleString("pt-BR")),"#d97706");
  }
  return c;
...
function vfRenderChips(ctx){
  if(ctx==="manual")setTimeout(_vfSyncScrollHint,0); // a fileira só tem largura depois que a aba aparece
  ...
  const b=g(ctx==="auto"?"#vf-badge-auto":"#vf-badge-manual");
  if(b){b.style.display=n?"":"none";b.textContent=String(n);}
}
function vfRemove(ctx,dim,v){
  const st=VF.st[ctx];if(!st)return;
  if(Array.isArray(st[dim])){ if(v&&dim!=="inicio"&&dim!=="cargo"&&dim!=="grupo")st[dim]=st[dim].filter(x=>x!==v);else st[dim]=[]; }
  ...
  else if(dim==="ocultarEncerradas")st.ocultarEncerradas=false; // "mostrar as encerradas"
...
// A categoria "parecida" vira SUGESTÃO visível e OPCIONAL — quem decide
// incluir é o usuário, e aí ela entra em TODOS os números (padrão Indeed).
function _vfRenderSugestoes(sugs,q){
  const antiga=document.getElementById("vf-sug-bar");if(antiga)antiga.remove();
  const s=(sugs||[])[0];if(!s||!s.n)return;
  const jl=g("#jlist");if(!jl||!jl.parentElement)return;
...
function vfAfterChange(ctx){
  ...
  if(ctx==="manual"){
    ...
    sSkip=0;sDone=false;sJobs=[];const jl=g("#jlist");if(jl)jl.innerHTML=mkSkels(6);loadSheetMeta(true);
    vfCountManual();
  } else vfAutoCount();
}
...
async function vfAutoCount(){
  const box=g("#vf-count-auto");if(!box)return;
  if(!autoSelectedSrc){box.style.display="none";return;}
  const seq=++_vfAutoSeq;
  box.style.display="block";box.innerHTML=`<span class="spin spin-sm"></span> ${esc(t('vf_counting'))}`;
  try{
    const d=await vfFetch("auto");if(!d||seq!==_vfAutoSeq)return;
    VF.fac.auto=d;
    const n=d.total||0;
    if(d.disponibilidade.email===0){box.className="vf-count vf-count-warn";box.innerHTML=`⏳ ${esc(t('vf_no_email_auto'))}`;}
    else if(n===0){box.className="vf-count vf-count-warn";box.innerHTML=`⚠️ ${esc(vfAtivos(VF.st.auto)?t('vf_zero'):t('vf_all_sent'))}`;}
    else{box.className="vf-count";box.innerHTML=`✅ <strong>${n.toLocaleString("pt-BR")}</strong> ${esc(t('vf_count_auto'))}${d.totalBase>n?` <span class="vf-dim">${esc(t('vf_of').replace("{m}",d.totalBase.toLocaleString("pt-BR")))}</span>`:""}`;}
    _vfWs2Sub(d);vfPonteSync();
    const pf=g("#pf-count");if(pf)pf.textContent=n.toLocaleString("pt-BR");
  }catch(e){if(seq===_vfAutoSeq)box.style.display="none";}
}
// Resumo curto pro contador da lista ("· 🔍 3 filtros")
function vfResumoCurto(ctx){const n=vfAtivos(VF.st[ctx]);return n?` · <span style="font-size:11px;color:var(--purple);font-weight:700">🔍 ${n} ${n===1?t('vf_filtro'):t('vf_filtros')}</span>`:"";}
// Snapshot dos filtros de um contexto — FONTE ÚNICA. O robô usa com
// email:true forçado (só manda pra quem tem e-mail; regra 13 "se realimenta
// com os mesmos filtros"); o 📡 Radar usa o do manual SEM forçar nada.
function vfSnapshot(ctx,extra){return Object.assign({},VF.st[ctx==="auto"?"auto":"manual"],extra||{});}
function vfSnapshotAuto(extra){return vfSnapshot("auto",Object.assign({email:true},extra||{}));}
// 🌉 v182 LOTE 9: quem montou "Flórida + Alimentação + $18+/h" na busca manual
// chegava no Passo 2 do robô com o painel ZERADO e tinha que repetir tudo —
// contra a régua da casa de reduzir cliques. O botão só aparece quando há
// filtro montado do outro lado.
function vfUsarFiltrosDaBusca(){
  const st=VF.st.manual;if(!st)return;
  VF.st.auto=Object.assign(_vfClone(st),{email:true}); // o robô só envia pra vaga com e-mail
  vfSave("auto");vfAfterChange("auto");vfPonteSync();
  toast(t('vf_ponte_ok'),"g");
}
function vfPonteSync(){
  const b=g("#btn-vf-ponte");if(!b)return;
  const n=vfAtivos(VF.st.manual,"manual");
  b.style.display=n?"":"none";
}
// O subtítulo do Passo 2 prometia "mês de início" — dimensão que NÃO existe
// nas duas fontes H-2B do robô (disponibilidade.inicio === 0 em jan2026 e
// jul2025). Agora ele cita só o que a fonte escolhida tem de verdade.
function _vfWs2Sub(d){
  const el=document.querySelector('[data-i18n="vf_ws2_sub"]');if(!el)return;
  const disp=d&&d.disponibilidade;
  if(!disp){el.textContent=t('vf_ws2_sub');return;}
  const dims=[["categoria","vf_sec_categoria"],["estado","vf_sec_estado"],["salario","vf_sec_salario"],["exp","vf_sec_exp"],
    ["temporada","vf_sec_temporada"],["inicio","vf_sec_inicio"],["cidade","vf_sec_cidade"],["cargo","vf_sec_cargo"],["vagas","vf_sec_vagas"]]
    .filter(([k])=>(disp[k]||0)>0&&(disp[k+"Distintos"]===undefined||disp[k+"Distintos"]>=2))
    .map(([,chave])=>t(chave).toLowerCase());
  el.textContent=dims.length?t('vf_ws2_dyn').replace("{d}",dims.join(", ")):t('vf_ws2_sub');
}
...
function _jobVisaTypeFront(jOrSrc){
  if(typeof jOrSrc==="string"){const s=jOrSrc.toLowerCase();if(s.includes("h2a"))return"h2a";if(s&&s!=="manual")return"h2b";return null;}
...
// ═══════════════════════════════════════════
let stmr;
function onSearch(){_vfSyncQClear();clearTimeout(stmr);stmr=setTimeout(()=>{const q=g("#q").value.trim();if(q!==fQ){_vfSetQ(q);sSkip=0;sDone=false;sJobs=[];loadSheetMeta(true);vfCountManual();}},350);}
...
function qEstadoPick(v){
  v=String(v||"").trim().toUpperCase();if(!v)return;
  const st=VF.st.manual;if(!st.estado.includes(v))st.estado.push(v);
  vfSave("manual");vfAfterChange("manual");
  toast("📍 "+_vfEstadoNome(v)+" "+t('vf_filtered'),"g");
}
function qCidadePick(v){
  v=String(v||"").trim();if(!v)return;
  const st=VF.st.manual;if(!st.cidade.includes(v))st.cidade.push(v);
  vfSave("manual");vfAfterChange("manual");
  toast("🏙️ "+v,"g");
}
...
function qSugPick(i){
  ...
  sSkip=0;sDone=false;sJobs=[];loadSheetMeta(true);vfCountManual();
}
function qSugKey(ev){
...
async function radarModal(){
  ...
  const _rChips=r?_vfChipsDe(Object.assign(_vfEmpty(),{email:false,ocultarEncerradas:false},r.filtros||{estado:r.estados||[],cidade:r.cidade?[r.cidade]:[],q:r.q||"",categoria:r.categoria?[r.categoria]:[]}),"manual"):[];
  ...
  ov.innerHTML=`<div style="background:var(--sf);border-radius:18px;padding:22px;max-width:400px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.25)">
    ...
    ${cur}
    <div style="font-size:12.5px;color:var(--t2);line-height:1.5;margin-bottom:10px">
      <b>${esc(t('radar_evaluated'))}</b><br>
      <span id="radar-agora" style="color:var(--t3)">${_vfChipList("manual").map(c2=>esc(c2.lbl)).join(" · ")||esc(t('radar_all'))}</span>
    </div>
...
async function radarCreate(){
  // 📡 v182 LOTE 9: ia só {estados, 1ª cidade, q, 1ª categoria} — salário,
  // vagas, mês, cargo, experiência, temporada e visto ficavam de fora, e o
  // botão fica na MESMA fileira do 🔍 Filtros ("salva o que eu filtrei" é a
  // leitura natural). Agora vai o snapshot INTEIRO, sem forçar e-mail.
...
/* ⭐ v134: FUNIL — limite diário atingido = mostrar o que ele está perdendo
   ...
   💳 v197 LOTE 15: quem JÁ PAGA lia "Assine um plano" e "Continuar amanhã de
   graça" ao bater o próprio limite — texto de conta grátis na cara de
   cliente. Agora a mensagem ramifica pelo estado REAL das DUAS dimensões que
   o /api/status manda (manualLimit/autoLimit são 0 quando aquele lado não
   está ativo — v172i), nunca por "tem plano":
     · sem nada ativo   → convite pra assinar (o de sempre);
     · só manual ativo  → Turbo/Máximo somam o robô ao que ele já tem;
     · só o automático  → o manual dele venceu: a data é o que importa, não
                          um upgrade de tier;
     · Máximo (topo) → NENHUM upsell: não existe pra onde subir. */
function limitUpsell(){
  const _manualOn=(U.manualLimit||0)>0, _autoOn=(U.autoLimit||0)>0;
  const _topo=String(U.plan||"")==="doublepro"&&_manualOn&&_autoOn;
  if(_topo||U.isAdmin)return; // nada a oferecer — seria só barulho
  ...
  const rest=(typeof sTotal!=="undefined"&&sTotal>1)?sTotal:null;
  let _titulo,_corpo,_cta;
  if(!_manualOn&&!_autoOn){
    _titulo=t('upsell_title');
    _corpo=rest?esc(t('upsell_left')).replace("{n}",rest.toLocaleString("pt-BR")):esc(t('upsell_left_generic'));
    _cta=t('upsell_cta');
  } else if(_manualOn&&!_autoOn){
    _titulo="Seu limite de hoje acabou";
    _corpo=esc(`Você usou os ${U.manualLimit} envios manuais de hoje. Os planos com robô (Turbo e Máximo) seguem enviando por você 24h, sem você precisar abrir o app.`);
    _cta="Ver planos com envio automático";
  } else {
    const _venc=_planVenceuTxt();
    _titulo="Seu limite de hoje acabou";
    _corpo=esc(`O envio automático continua ativo (${U.autoLimit}/dia), mas o seu envio manual${_venc?` venceu em ${_venc}`:" não está ativo"}. Renovando o manual, você volta a se candidatar você mesmo, além do robô.`);
    _cta="Ver planos";
  }
  const ov=document.createElement("div");
  ov.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.65);z-index:500;display:flex;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(4px)";
  ov.innerHTML=`<div style="background:var(--sf);border-radius:18px;padding:24px;max-width:380px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,.25);text-align:center">
    ...
    <div style="font-size:17px;font-weight:800;margin-bottom:8px">${esc(_titulo)}</div>
    <div style="font-size:13.5px;color:var(--t2);line-height:1.55;margin-bottom:16px">${_corpo}</div>
    <button class="btn btn-primary" style="width:100%" onclick="this.closest('div[style*=fixed]').remove();sv('plans')">⭐ ${esc(_cta)}</button>
...
function setSort(s){fSort=s;["rand:random","match:match","wage:wage","start:start","desc:desc"].forEach(p=>{const[i,v]=p.split(":");g("#so-"+i)?.classList.toggle("on",s===v);});sSkip=0;sDone=false;sJobs=[];loadSheetMeta(true);}
  ...
  try{
    ...
    if(!d.jobs?.length){sDone=true;
      ...
      var _fj=d.jobs.filter(function(j){
        ...
        // v182 LOTE 10: sem plano ativo o endereço vem MASCARADO — comparar
        // máscara com máscara daria falso "já contatado". O corte de verdade
        // (regra 8) roda no servidor, sobre a linha real, e continua valendo.
        if(j.email&&!j.emailBloqueado&&typeof empregadorStatus==="function"&&empregadorStatus(j.email))return false;
        return true;
      });
      ...
      sSkip+=d.jobs.length;sTotal=d.total||sJobs.length;sTrueTotal=d.remainingTotal||sTotal;
...
// v223 (ordem do dono, "só planilhas a partir de agora"): não existe mais
// nenhuma consulta em tempo real ao DOL — o detalhe da vaga é 100% o que já
// está na planilha carregada (cache em memória ou /api/sheet-meta), sempre
// síncrono, sem round-trip nem estado de "carregando detalhes".
function selSheetJob(cn){
  ...
  // 1) Cache em memória (v38: com guarda de empregador contatado)
  if(sCache[cn]){_showJobOrSweep(cn,sCache[cn]);return;}
  // 2) O que já veio de /api/sheet-meta pra esta planilha
  const localMeta=sJobs.find(x=>x.id===cn||x.caseNum===cn);
  if(localMeta){
    const j={...localMeta,fromSheet:true};
    sCache[cn]=j;
    _showJobOrSweep(cn,j);
    return;
  }
  showDetail(`<div class="jd-content"><div class="alert al-amber"><i class="ti ti-alert-triangle"></i><div>Vaga não encontrada nesta planilha.<br><strong>Case:</strong> ${esc(cn)}</div></div></div>`);
}
...
function mkDetailHTML(j){
  ...
  return`<div class="jd-content">
    ...
    <div class="info-grid">
      ...
      ${j.hasEmail&&j.emailBloqueado?`<div class="info-box" style="grid-column:1/-1;border-color:var(--amberb);background:var(--amberl)"><div class="info-lbl">📧 Email</div><div class="info-val" style="font-size:12px;color:#92400e;font-weight:700;cursor:pointer" onclick="location.reload()">🔒 ${esc(t('email_locked'))}</div></div>`
...
async function doSend(){
  ...
  try{
    ...
    U.todaySentManual=d.todaySent||U.todaySentManual+1;U.manualRemaining=typeof d.remaining==="number"?d.remaining:Math.max(0,(U.manualLimit||0)-U.todaySentManual);U.manualLimit=d.dailyLimit??U.manualLimit;
    ...
    if(j?.id){
      APPLIED.add(j.id);
      if(j.caseNum||j.id)_sentAll.add(j.caseNum||j.id); // qualquer planilha
    ...
    HIST.unshift({appId:d.appId,jobId:j?.id,job:j?.title||subj,company:j?.company||"",to,date:new Date().toLocaleString("pt-BR"),type:"manual",sheetSource:tab,caseNum:j?.caseNum||j?.id||"",threadId:d.threadId,msgId:d.messageId,jobSnapshot:{title:j?.title,company:j?.company,city:j?.city,state:j?.state,wage:j?.wage,visa:j?.visa||j?.visaType,start:j?.start,end:j?.end,desc:j?.desc,sourceEmail:to}});
    updHistBadge();
...
function loadProfile(){
  ...
  // ou o legado "pro"). O usuário via "⭐🤖 Turbo" com 0 envios manuais por dia
  // e nenhum "Nd restantes" do lado (a chip só olhava manualExpires). Passa a
  // usar planBadgeHTML(), a fonte ÚNICA já honesta do site: mostra ✋ Manual e/ou
...
function _warmupBadgeHTML(sentToday,cap){
  if(cap==null)return"";
  // 🩺 v221 (auditoria ao vivo do dono, 20/09/2026): pro admin o teto por dias
  // (15/40/100) NUNCA foi o limite real — o funcional é ADMIN_AUTO_DAILY_
  // LIMIT_PER_SENDER=450/dia (regra 13a2: aquecimento é só PREFERÊNCIA de
  // rodízio, nunca trava). Mostrar "276/15" pro admin sugeria um teto furado
  // (caso real: Gmail recém-conectado neste ambiente já tinha enviado bem
  // mais que o dia-tier, sem nada de errado). Mantém o selo (o dia-tier ainda
  // vale como preferência entre os Gmails do admin) mas sem a fração enganosa.
  if(U.isAdmin){
    return`<div style="font-size:9.5px;color:#d97706;background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.3);border-radius:6px;padding:2px 6px;margin-top:3px;display:inline-block">🌱 Ainda no período de aquecimento — ${sentToday||0} enviados hoje (só preferência de rodízio, nunca trava; teto real é o do Admin acima)</div>`;
  }
...
function _renderProfileSenderSection(){
  ...
  // 📧 ORDEM DO DONO (13/09/2026): Gmail extra é SÓ Máximo (2 e-mails);
  // Manual/Turbo têm 1 (o principal); sem plano, 0. senderMax vem do servidor
  // (getMaxSenders) — o front nunca decide plano.
  const senderMax=(typeof U.senderMax==="number")?U.senderMax:1; // total permitido (principal + extras)
  ...
  // propósito: aquele só aparece pra Máximo, e o principal é de todo mundo
...
function _peSaveDraftNow(){
  try{
    ...
    const temAlgo=d.subjects.some(Boolean)||d.bodies.some(Boolean);
...
async function saveProfileFromEditor(){
  ...
  // 🚨 v172l (ordem do dono, 13/09/2026 — "nome do perfil e descrição não
  // precisa, são coisas que tinha antigamente e agora não tem mais"):
  // nome deixou de ser digitado — é sempre "Perfil H-2B"/"Perfil H-2A"
  // (1 perfil por tipo de visto já existe, não precisa de nome distintivo).
  const name=editingVisaType==="h2a"?"Perfil H-2A":"Perfil H-2B";
...
async function doClearHist(ovEl){
  ...
  // _sentAll/_sentJan/_sentJul (por número da vaga, usados pelo filtro da
  // lista). Resultado real: logo após resetar, as vagas voltavam… e
  // SUMIAM de novo na primeira varredura, ou bloqueavam no clique — o
  // CONTRÁRIO da regra ("resetou, tudo volta"). Agora o reset zera os
  // conjuntos locais, espera o servidor limpar e re-sincroniza.
  _empSent=new Set();
  _sentAll=new Set();_sentJan=new Set();_sentJul=new Set();
...
function loadAutoView(){
  ...
  loadTabCounts();
  if(autoSelectedSrc){vfRenderChips("auto");vfAutoCount();}
  recalcInterval();
...
function selectSource(src){
  autoSelectedSrc=src;
  // v173: filtros por planilha (VF) — carrega o que a pessoa tinha nesta fonte
  vfLoad("auto");vfRenderChips("auto");
  Promise.resolve(_ensureCatLabels()).then(()=>vfRenderChips("auto"));
...
// ⏱️ v197 LOTE 15 — O INTERVALO DO ROBÔ NUMA CONSTANTE SÓ.
// O motor real é o `calcSmartInterval` (mod-engine-core.js): 6,5 a 7,5
// minutos com jitter, média 7 — subiu pra ~7min no v118 por ordem do dono
// ("gente demais batendo nas mesmas empresas"). A tela continuava dizendo
// "5–6 min" em 5 lugares e, pior, CALCULAVA a previsão de término com 5,5
// min por envio: ~27% otimista, num número que a pessoa usa pra se
// planejar. Mexeu no motor, mexe aqui — e em lugar nenhum mais.
const AUTO_INT_MIN=6.5, AUTO_INT_MAX=7.5, AUTO_INT_AVG=7;
const AUTO_INT_LBL=`${AUTO_INT_MIN.toString().replace(".",",")}–${AUTO_INT_MAX.toString().replace(".",",")} min`;

// ── recalcInterval: previsão com o intervalo REAL do motor (~7min) ──────
function recalcInterval(){
  ...
  const autoLimit=U.isAdmin?(U.autoLimit||450):(U.autoLimit??0);
  ...
  if(U.isAdmin){
    ...
    // conta continua recebendo 1 e-mail a cada ~7min, só que são 2 contas).
    const extraSenders=(U.senderEmails||[]).filter(s=>s&&s.active!==false&&!s.tokenExpired&&!s.blocked).length;
    const activeSenders=Math.min(3,Math.max(1,1+extraSenders));
    const avgMin=AUTO_INT_AVG/activeSenders;
    const totalMin=Math.round(autoLimit*avgMin);
    const h=Math.floor(totalMin/60),m=totalMin%60;
    const dur=h>0?(m>0?`${h}h ${m}min`:`${h}h`):`${m}min`;
    if(el)el.textContent=activeSenders>1?`⏱️ 1 e-mail a cada ${(AUTO_INT_MIN/activeSenders).toFixed(1)}–${(AUTO_INT_MAX/activeSenders).toFixed(1)} minutos (${activeSenders} Gmails revezando)`:`⏱️ 1 e-mail a cada ${AUTO_INT_LBL}`;
...
function openPreflightModal(){
  ...
  if(!U.isAdmin&&(U.autoLimit||0)<=0){
    toast("⚠️ Você precisa de um plano com envio automático (Turbo ou Máximo) pra usar o robô.","r",7000);
    sv("plans");return;
  }
  if(!U.gmailConnected){
  ...
  const avgMinPf=AUTO_INT_AVG/_activeSenders;
  ...
  const intervalLbl=_activeSenders>1?`${(AUTO_INT_MIN/_activeSenders).toFixed(1)}–${(AUTO_INT_MAX/_activeSenders).toFixed(1)} min`:AUTO_INT_LBL;

  // v173: contagem EXATA da fila (a mesma do Passo 2 — /api/vagas/filtros com
  // todos os filtros e já sem as vagas enviadas/na fila)
  const _pfN=VF.fac.auto?VF.fac.auto.total:null;
  ...
  g("#pf-subtitle").textContent=`${autoLimit} e-mails/dia • intervalo ${intervalLbl}${_activeSenders>1?` (${_activeSenders} Gmails revezando)`:""} • servidor sempre ligado`;
...
async function startAuto(overrideStartH, overrideEndH, _mode="now"){
  ...
  try{
    ...
    while(allCases.length<limit){
      // v173: os MESMOS filtros do Passo 2 (VF), pelo mesmo motor da contagem
      const p=vfParams("auto");p.set("sheet",autoSelectedSrc);p.set("skip",String(sk));p.set("top",String(Math.min(pageSize,limit-allCases.length)));p.set("hideSent","1");
    ...
    const resp=await fetch("/api/auto/start",{
      ...
      body:JSON.stringify({
        ...
        category:VF.st.auto.categoria.join(",")||"all",
        startH,
        endH,
        mode:_mode,
        senders:getSelectedAutoSenders(),
        profileId:autoSelectedProfileId||null,
        // v173: snapshot dos filtros — o servidor usa o MESMO motor no refill (regra 13)
        filtros:vfSnapshotAuto({limit})
      })
    });
...
// venceu dia tal"): mesmo cálculo do server planGateMsg(), version client —
// usada pelos 2 gates (manual e automático) pra nunca mostrar um "expirou"
// genérico quando já se sabe a data exata.
function _planVenceuTxt(){
...
// 🔒 v172h (ordem do dono, 12/09/2026 — "esse botão deve ser grande,
// visível"): virou um cartão de verdade (não mais uma pilulazinha de 1
// linha) com botão grande de ação — "Ver planos" pra quem não tem plano
// (o botão de conectar Gmail NEM EXISTE pra quem é free, só reaparece
// depois de assinar) e "Conectar meu Gmail" em destaque pra quem já pagou.
  ...
  if(!U.isAdmin&&U.needsPlan){
    ...
    el.innerHTML=`<div style="display:flex;align-items:center;gap:12px;width:100%">
      ...
      <div style="flex:1;min-width:150px">
        <div style="font-size:14px;font-weight:800;color:#dc2626">Plano necessário pra enviar</div>
        <div style="font-size:12px;color:#7f1d1d;opacity:.85;margin-top:2px">${esc(_sub)}</div>
      </div>
      <button onclick="sv('plans')" class="btn btn-primary" style="flex-shrink:0;padding:11px 18px;font-size:13.5px">Ver planos <i class="ti ti-arrow-right"></i></button>
    </div>`;
    el.style.background="rgba(239,68,68,.1)";el.style.border="1.5px solid rgba(239,68,68,.35)";el.style.color="#dc2626";
  } else {
    el.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px;width:100%">
      <div style="display:flex;align-items:center;gap:12px">
        <div style="width:42px;height:42px;border-radius:12px;background:rgba(245,158,11,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="ti ti-mail-exclamation" style="font-size:21px;color:#b45309"></i></div>
        <div style="flex:1;min-width:150px">
          <div style="font-size:14px;font-weight:800;color:#b45309">Conecte seu Gmail pra começar a enviar</div>
          <div style="font-size:12px;color:#78350f;opacity:.85;margin-top:2px">Esse e-mail vai ser o único usado pra enviar suas candidaturas — não dá pra trocar depois.</div>
        </div>
      </div>
      <button onclick="connectGmailForSending('jobs')" class="btn btn-primary w100" style="justify-content:center;padding:13px;font-size:14.5px;font-weight:800"><i class="ti ti-brand-google" style="font-size:17px"></i> Conectar meu Gmail agora</button>
    </div>`;
...
// 🔒 v172h (ordem do dono, 12/09/2026 — "esse botão deve ser grande,
// visível... na tela venceu dia tal"): a pílula (#auto-free-note-lbl)
// continua como resumo compacto no hero; #auto-gate-card ganhou o cartão
// GRANDE de ação (mesmo padrão do updateManualSendGate) só nos 2 estados
// que bloqueiam envio — nos demais (admin liberado, Turbo/Máximo com
// Gmail conectado) o cartão some e só a pílula informativa fica visível.
function _updateAutoFreeBanner(){
  const lbl=g("#auto-free-note-lbl");const card=g("#auto-gate-card");
  if(!lbl)return;
  ...
  if(U.isAdmin&&!U.gmailConnected){
    lbl.innerHTML='<i class="ti ti-mail-exclamation"></i> <strong>Conecte seu Gmail</strong> (Perfil → Admin) pra poder enviar';
    lbl.style.background="rgba(245,158,11,.12)";lbl.style.borderColor="rgba(245,158,11,.4)";lbl.style.color="#b45309";
    if(card){card.style.display="flex";card.style.background="rgba(245,158,11,.12)";card.style.border="1.5px solid rgba(245,158,11,.4)";
      card.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px;width:100%">
        <div style="display:flex;align-items:center;gap:12px">
          <div style="width:42px;height:42px;border-radius:12px;background:rgba(245,158,11,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="ti ti-mail-exclamation" style="font-size:21px;color:#b45309"></i></div>
          <div style="flex:1;min-width:150px"><div style="font-size:14px;font-weight:800;color:#b45309">Conecte seu Gmail pra poder enviar</div><div style="font-size:12px;color:#78350f;opacity:.85;margin-top:2px">Perfil → Admin, ou pelo botão abaixo.</div></div>
        </div>
        <button onclick="connectGmailForSending('auto')" class="btn btn-primary w100" style="justify-content:center;padding:13px;font-size:14.5px;font-weight:800"><i class="ti ti-brand-google" style="font-size:17px"></i> Conectar meu Gmail agora</button>
      </div>`;
    }
  } else if(U.isAdmin){
    ...
    const _sub=_planGateSubTxt("assine Turbo ou Máximo pra continuar.")||"Assine Turbo ou Máximo pra usar o automático.";
    ...
    if(card){card.style.display="flex";card.style.background="rgba(239,68,68,.1)";card.style.border="1.5px solid rgba(239,68,68,.35)";
      card.innerHTML=`<div style="display:flex;align-items:center;gap:12px;width:100%">
        <div style="width:42px;height:42px;border-radius:12px;background:rgba(239,68,68,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="ti ti-lock" style="font-size:21px;color:#dc2626"></i></div>
        <div style="flex:1;min-width:150px"><div style="font-size:14px;font-weight:800;color:#dc2626">Plano necessário pro automático</div><div style="font-size:12px;color:#7f1d1d;opacity:.85;margin-top:2px">${esc(_sub)}</div></div>
        <button onclick="sv('plans')" class="btn btn-primary" style="flex-shrink:0;padding:11px 18px;font-size:13.5px">Ver planos <i class="ti ti-arrow-right"></i></button>
      </div>`;
    }
  } else if(!U.gmailConnected){
    lbl.innerHTML='<i class="ti ti-mail-exclamation"></i> <strong>Conecte seu Gmail</strong> pra começar a enviar';
    lbl.style.background="rgba(245,158,11,.12)";lbl.style.borderColor="rgba(245,158,11,.4)";lbl.style.color="#b45309";
    if(card){card.style.display="flex";card.style.background="rgba(245,158,11,.12)";card.style.border="1.5px solid rgba(245,158,11,.4)";
      card.innerHTML=`<div style="display:flex;flex-direction:column;gap:10px;width:100%">
        <div style="display:flex;align-items:center;gap:12px">
          <div style="width:42px;height:42px;border-radius:12px;background:rgba(245,158,11,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="ti ti-mail-exclamation" style="font-size:21px;color:#b45309"></i></div>
          <div style="flex:1;min-width:150px"><div style="font-size:14px;font-weight:800;color:#b45309">Conecte seu Gmail pra começar a enviar</div><div style="font-size:12px;color:#78350f;opacity:.85;margin-top:2px">Esse e-mail vai ser o único usado pra enviar suas candidaturas — não dá pra trocar depois.</div></div>
        </div>
        <button onclick="connectGmailForSending('auto')" class="btn btn-primary w100" style="justify-content:center;padding:13px;font-size:14.5px;font-weight:800"><i class="ti ti-brand-google" style="font-size:17px"></i> Conectar meu Gmail agora</button>
      </div>`;
    }
  } else if(U.plan==="doublepro"){
    lbl.innerHTML=`<i class="ti ti-crown"></i> <strong>Máximo</strong> — ${U.autoLimit} envios/dia com 2 Gmails`;
    lbl.style.background="rgba(99,102,241,.1)";lbl.style.borderColor="rgba(99,102,241,.3)";lbl.style.color="#4f46e5";
    if(card)card.style.display="none";
  } else {
    lbl.innerHTML=`<i class="ti ti-star"></i> <strong>Turbo</strong> — ${U.autoLimit} envios/dia automático`;
...
function updateAutoUI(){
  ...
  const daysLbl=g("#dash-prog-days-lbl");if(daysLbl){
    const _lim=U.autoLimit??0;const _daysEst=_lim>0&&qs>0?Math.ceil(qs/_lim):null;
  ...
  // ETA = vagas restantes × intervalo real (admin usa o seu; usuário, ~7min).
  ...
  if(active&&(j.currentJob||_prev.length)){
    ...
    if(_eta){
      const _iv=window._autoIntervalSecs||(AUTO_INT_AVG*60);
...
// Infinite scroll
const obs=new IntersectionObserver(entries=>{if(!entries[0].isIntersecting)return;if(!sLoading&&!sDone)loadSheetMeta();},{threshold:0.1,rootMargin:"200px"});
...
function renderAutoSenders(){
  ...
  // 📧 ORDEM DO DONO (13/09/2026 — "deixa claro que só funciona com mais de
  // 1 e-mail dependendo da conta"): quantos e-mails cabem é do PLANO —
  // grátis 0 · Manual/Turbo 1 · Máximo 2 · admin 6 (getMaxSenders no
  // servidor; senderMax no /api/status). Legenda sempre visível diz o teto
  // real de QUEM está logado, pra ninguém achar que é bug quando só vê 1
  // e-mail, nem confundir "meu" limite com o do admin.
  const hint=g("#auto-senders-hint");
  const max=(typeof U.senderMax==="number")?U.senderMax:1;
  if(hint)hint.textContent=(U.isAdmin?t('snd_hint_admin'):max>=2?t('snd_hint_dp'):max===1?t('snd_hint_1'):t('snd_hint_0')).replace("{n}",String(max));
  const listWrap=g("#auto-senders-list-wrap");
  if(all.length<2){
    // Só 1 e-mail disponível: mostra a legenda mesmo assim (explica o
    // porquê), esconde a lista/checkboxes (não faz sentido escolher entre
    // 1 opção só, nem explicar "revezamento" que não vai acontecer).
    box.style.display="block";
    if(listWrap)listWrap.style.display="none";
    list.innerHTML="";
    window._autoSenders=null;
    return;
  }
  box.style.display="block";
...
function renderNextStep(){
  ...
  let step=null;
  // ⚠️ v185 LOTE 3: quem NÃO tem plano pago ativo não consegue enviar NADA
  // (free é 0/0 desde o v172) — mandar essa pessoa "buscar vagas" ou
  // "ativar o automático" é mandar pra uma parede. O 1º ramo passa a ser o
  // gate oficial do servidor (U.needsPlan, o MESMO campo que o /api/send e o
  // /api/auto/start usam). NUNCA usar U.plan==='free' aqui: getPlan() devolve
  // "vipro" pra quem só tem o automático, e foi essa a armadilha do v177-FIX8.
  if(!U.isAdmin && U.needsPlan){
    step={icon:"⭐",ic:"var(--purple)",bg:"linear-gradient(135deg,#f5f3ff,#ede9fe)",bd:"var(--purpleb)",title:t('ns0_t'),sub:t('ns0_s'),cta:t('ns0_c'),act:"goPlans"};
  } else if(!hasProfile){
  ...
  } else if((U.manualLimit||0)>0 && (U.manualRemaining||0)<=0){
...
let _pendingOrderCache=undefined; // undefined=ainda não checou, null=checou e não tem nada a contar, obj=pedido
let _pendingOrderCacheEm=0;         // quando o cache foi montado (envelhece em 5min)
let _pedidoCanceladoDispensado=null; // id que o usuário fechou no X (só nesta sessão)
const WA_SUPORTE="https://wa.me/5553981453496";
async function renderPendingOrderCard(){
  const box=g("#home-pending-order");if(!box)return;
  if(!U.connected){box.style.display="none";return;}
  // v185 LOTE 3: o cache vivia pra sempre — pedido aprovado (ou cancelado
  // pelo robô do v178) continuava anunciado como "em análise" até a pessoa
  // recarregar o app. Agora ele envelhece em 5min e é invalidado na hora em
  // que o /api/status muda plano/gate (applyStatus).
  if(_pendingOrderCache!==undefined&&Date.now()-_pendingOrderCacheEm>5*60_000)_pendingOrderCache=undefined;
  if(_pendingOrderCache===undefined){
    ...
    try{
      ...
      const d=await jsonSafe(r);
      // ⚠️ Antes era .find(status==="pendente") — dependia da ORDEM em que a
      // lista chegou e IGNORAVA o pedido cancelado: quem teve o pedido
      // recusado (comprovante com valor divergente é cancelado sozinho desde
      // o v178) simplesmente sumia da Home, sem NENHUM aviso, e ficava
      // esperando um plano que nunca vinha. Agora vale o pedido MAIS RECENTE
      // entre pendente e cancelado, e o cancelado só aparece por 7 dias.
      const cand=(d?.pedidos||[]).filter(p=>p&&(p.status==="pendente"||(p.status==="cancelado"&&(Date.now()-(p.createdAt||0))<7*86400_000)))
        .sort((a,b)=>(b.createdAt||0)-(a.createdAt||0));
      _pendingOrderCache=cand[0]||null;
    }catch(e){_pendingOrderCache=null;}
    _pendingOrderCacheEm=Date.now();
  ...
  box.style.display="block";
  box.innerHTML=`<div style="background:${cancelado?"linear-gradient(135deg,#fef2f2,#fee2e2)":"linear-gradient(135deg,#fffbeb,#fef3c7)"};border:1.5px solid ${cancelado?"#fca5a5":"#fcd34d"};border-radius:14px;padding:13px 14px;display:flex;align-items:flex-start;gap:12px">
    <div style="width:42px;height:42px;border-radius:12px;background:#fff;display:flex;align-items:center;justify-content:center;font-size:21px;flex-shrink:0;box-shadow:0 2px 8px rgba(0,0,0,.06)">${cancelado?"❌":"⏳"}</div>
    <div style="flex:1;min-width:0">
      <div style="font-size:10px;font-weight:800;color:${cancelado?"#991b1b":"#92400e"};text-transform:uppercase;letter-spacing:.05em;margin-bottom:1px">${cancelado?esc(t('po_canc')):esc(t('po_pend'))}</div>
      <div style="font-size:14px;font-weight:800;color:var(--text);line-height:1.25">${esc(planLbl)} ${cancelado?esc(t('po_canc_t')):esc(t('po_pend_t'))}</div>
      <div style="font-size:11.5px;color:var(--t2);line-height:1.35;margin-top:2px">${cancelado
        ?(motivo?esc(t('po_motivo'))+": <strong>"+esc(motivo)+"</strong><br>":"")+esc(t('po_canc_s'))
...
// O card vermelho não pode virar permanente: o X some com ele nesta sessão
// (e o pedido continua visível em "Meus pagamentos", que é o registro real).
function dispensarPedidoCancelado(id){
...
function renderHome(){
  ...
  if(ac){
    ...
    if(U.autoJob?.active){
      ...
      // v185 LOTE 3: pra quem não tem plano ativo, "Inicie o seu Automático"
      // é um convite pra um cadeado — o card-herói agora fala a verdade e
      // leva pra Planos (mesmo gate do servidor, U.needsPlan).
      const _semPlano=!U.isAdmin&&U.needsPlan;
      if(at){at.textContent=_semPlano?t('ha_semplano_t'):"🤖 Inicie o seu Automático";at.style.color="#fff";}
      if(as){as.innerHTML=_semPlano?esc(t('ha_semplano_s')):"Envie currículos enquanto você trabalha. Não garantimos a vaga — mas garantimos que seu currículo <strong>chegue ao empregador</strong>.";as.style.color="rgba(255,255,255,.92)";}
      if(ai)ai.textContent="🤖";
    }
  }
}
...
// v209 (dono): demo falso removido — landing agora usa screenshots reais na seção "Veja o site por dentro" (index.html)
;
/* ═══ bloco extraído ═══ */
...
const LANG_DICT = {
  pt: {
     // 🌐 v137b
    "ns0_t":"Escolha seu plano pra começar","ns0_s":"Sem plano ativo o envio fica bloqueado — nem manual, nem automático.","ns0_c":"Ver planos","ha_semplano_t":"⭐ Escolha seu plano","ha_semplano_s":"O envio de candidaturas (manual e automático) precisa de um plano ativo.","po_pend":"Pedido em análise","po_pend_t":"em revisão","po_pend_s":"Enviado · confirmação após revisão do admin","po_canc":"Pedido cancelado","po_canc_t":"não foi aceito","po_canc_s":"Você pode fazer um pedido novo com o valor certo.","po_motivo":"Motivo","po_refazer":"Fazer novo pedido","po_wa":"Falar no WhatsApp","po_fechar":"Fechar aviso","pd_conf":"✅ Confirme seus dados de contato","pd_errado":"Algum dado está errado?","pd_corrigir":"Corrigir no meu Perfil","pd_nome":"Nome","pd_wpp":"WhatsApp","pd_cidade":"Cidade","pd_toast":"Atualize seus dados aqui e volte em Planos pra concluir o pedido.",
    "ns1_t":"Crie seu perfil de candidatura","ns1_s":"\u00c9 o que vai nos e-mails para as empresas. Leva 1 minuto.","ns1_c":"Criar perfil","ns2_t":"Anexe seu curr\u00edculo (PDF)","ns2_s":"Sem curr\u00edculo anexado, suas candidaturas n\u00e3o saem.","ns2_c":"Anexar","ns3_t":"Tudo pronto! Comece a se candidatar","ns3_s":"Seu perfil est\u00e1 completo. Envie sua primeira candidatura de hoje.","ns3_c":"Buscar vagas","ns4_t":"Voc\u00ea atingiu o limite de hoje","ns4_s":"Vire VIP e envie at\u00e9 100 candidaturas por dia.","ns4_c":"Ver planos","ns5_t":"Ative o Envio Autom\u00e1tico","ns5_s":"Deixe o sistema enviar candidaturas enquanto voc\u00ea trabalha.","ns5_c":"Ativar","logs_none":"Nenhum log ainda","logs_none_s":"Os logs aparecem aqui quando voc\u00ea usar o Envio Autom\u00e1tico", // 🌐 v137: dinâmicos da varredura E2E
    "g_1":"Envio Autom\u00e1tico","g_2":"Configure e deixe o sistema trabalhar por voc\u00ea","g_7":"Curr\u00edculos","g_30":"seu curr\u00edculo (PDF)","g_31":"texto do e-mail","g_32":"at\u00e9 2 perfis: um H-2B e um H-2A","g_33":"Anexa seu curr\u00edculo PDF em cada candidatura","g_34":"Define o texto do e-mail em ingl\u00eas","g_35":"Essencial para o Envio Autom\u00e1tico funcionar","g_36":"Curr\u00edculo usado:","g_39":"Sem expiração · 450 envios/dia por Gmail conectado · intervalo configurável","roi_vipro":"Turbo — 200/dia (manual + automático)","roi_dp":"Máximo — 350/dia (manual + automático)","pf_gmail_extra_max":"🔒 Gmail Extra — exclusivo Máximo","pf_vip_infinito":"VIP Infinito","g_40":"segundos entre cada envio","g_41":"3min (padr\u00e3o)","g_42":"Adicione Gmails acima para configurar limites","g_44":"Aten\u00e7\u00e3o:","gmail_contas":"Enviar muitas candidaturas por um Gmail só pode gerar bloqueio temporário pelo Google. VIP e Turbo enviam pelo seu Gmail cadastrado (1 conta); o Máximo reveza entre 2 Gmails, o que reduz o volume por conta e o risco de bloqueio. O risco de bloqueio é de","g_46":"responsabilidade do usu\u00e1rio","g_49":"vagas pra voc\u00ea","g_50":"\ud83c\udfaf Seu perfil ser\u00e1 usado em todos os envios:","g_51":"\u25b6 Come\u00e7ar agora","g_52":"Envia sem parar 24/7. Reseta \u00e0 meia-noite e continua at\u00e9 zerar a fila.","g_53":"\ud83d\udd50 Agendar hor\u00e1rio","g_54":"Envia das X \u00e0s Y horas todo dia. Fora do hor\u00e1rio fica pausado.","g_55":"Iniciar \u00e0s","g_56":"Parar \u00e0s","g_62":"Pagamento via PIX","g_63":"*obrigat\u00f3rio","g_64":"Print da confirmação do Pix do seu pagamento","g_65":"Toque para selecionar o comprovante","g_66":"JPG, PNG, PDF \u2014 m\u00e1x 5MB","g_68":"Para d\u00favidas, entre em contato:","g_69":"1 empresa confirmar = voc\u00ea est\u00e1 nos EUA \u2708\ufe0f","g_128":"desaparecer de qualquer lugar p\u00fablico","g_129":"fazer login novamente com o mesmo e-mail","g_150":"Este perfil ser\u00e1 usado nos envios manuais e autom\u00e1ticos","g_151":"curr\u00edculo (PDF)","g_152":"\u2460 Informa\u00e7\u00f5es B\u00e1sicas","g_154":"Escolha um \u00edcone para o perfil:","g_155":"\u2461 Curr\u00edculo &amp; Cover Letter (PDF)","g_156":"\ud83d\udcc4 Curr\u00edculo (PDF)","g_157":"\u2705 Curr\u00edculo vinculado a este perfil","g_158":"\ud83d\udce4 Novo arquivo \u2014 ser\u00e1 enviado ao salvar","g_159":"Clique ou arraste um PDF para fazer upload","g_160":"M\u00e1x. 5MB","g_161":"ou escolha da sua conta","g_163":"apenas nas vagas do tipo de visto deste perfil","g_166":"Vari\u00e1veis:","g_168":"\u2699\ufe0f Vari\u00e1veis \u2014 clique para copiar:","g_170":"Selecione as categorias para as quais este perfil ser\u00e1 usado automaticamente","g_171":"\u2464 Configura\u00e7\u00e3o","g_172":"Prote\u00e7\u00f5es sempre ativas:","g_173":"n\u00e3o podem ser desativados","g_190":"Crie seu primeiro Perfil de Curr\u00edculo para","g_191":"come\u00e7ar a enviar candidaturas","g_204":"S\u00f3 precisa estar logado uma vez.","g_212":". É por ela que conferimos o seu pagamento.","g_178":"Só o plano Máximo pode conectar um 2º Gmail de envio (o robô reveza entre os dois e reduz o risco de bloqueio). VIP e Turbo usam 1 e-mail; sem plano não dá pra vincular Gmail.","g_188":". Voc\u00ea pode ter","g_189":"o perfil que voc\u00ea escolher no Passo 3 do assistente","g_193":"Gmails de envio","g_194":"10 envios GR\u00c1TIS/dia para todos!","g_198":"\ud83d\udcec Pr\u00f3ximas candidaturas","g_199":"\u2014 Ver\u00e3o","g_202":"O envio alterna","g_203":"O autom\u00e1tico zerou a fila? Resetar enviados","g_205":"Hor\u00e1rio (Bras\u00edlia)","g_207":"🧾 Meus pedidos","g_210":"📸 Comprovante do pagamento","g_211":"📅 Data em que você pagou","g_239":"Configura\u00e7\u00f5es","g_239b":"Sess\u00e3o","g_239c":"Mais","tut_center":"Central de Tutoriais","settings_adv":"Configura\u00e7\u00f5es avan\u00e7adas","g_241":"Zona de perigo","g_256":"Planilhas compat\u00edveis","g_257":"Categorias de vaga","pe_eng_lbl":"🗣️ Seu inglês (opcional)","pe_eng_none":"Não falo","pe_eng_basic":"Básico","pe_eng_inter":"Intermediário","pe_eng_adv":"Avançado","pe_cnh_lbl":"Tenho CNH (carteira de motorista)","pe_cover_note":"📝 A <strong>cover letter</strong> (carta de apresentação) <strong>NÃO é obrigatória</strong> — só o <strong>résumé</strong> (seu currículo em PDF, logo acima) já basta pra se candidatar. Se tiver uma, ela vai junto e pode ajudar; se não tiver, pule esta parte sem problema.","pe_subj_title":"③ Título do e-mail (assunto)","pe_subj_help":"O <strong>título</strong> é a linha que o empregador vê na caixa de entrada antes de abrir — curto e direto, em inglês, ex.: <em>“Application for {vaga} — {nome}”</em>. Escreva <strong style='color:var(--red)'>3 versões diferentes</strong> (os 3 campos já estão abertos; pode adicionar mais). O sistema alterna entre elas a cada envio pra não parecer spam.","pe_count0":"0 de 3 preenchidos","pe_subj_add":"Adicionar outro título","pe_subj_warn":"Escreva pelo menos <strong>3 títulos diferentes</strong> (os campos com *) — é a proteção contra bloqueio por spam.","pe_body_title":"④ Corpo do e-mail (a mensagem)","pe_body_help":"O <strong>corpo</strong> é a mensagem completa que o empregador lê ao abrir: quem você é, a vaga que quer, sua experiência e como falar com você. Em inglês, com as suas palavras. Escreva <strong style='color:var(--red)'>3 versões diferentes</strong> (os 3 campos já estão abertos); as variáveis abaixo são trocadas automaticamente em cada envio.","pe_body_add":"Adicionar outro texto","pe_body_warn":"Escreva pelo menos <strong>3 textos diferentes</strong> (os campos com *) — é a proteção contra bloqueio por spam.", // 🌐 v136: varredura final (auto)
    ...
    "au_t":"Regra de conta única — leia antes de entrar","au_b":"Cada pessoa pode ter uma conta no H2BApply. Criar uma segunda conta — mesmo com outro e-mail — pode causar <strong>banimento permanente das duas contas</strong>, sem devolução de nada. Lembre-se: o nome no seu currículo é sempre o mesmo, e o sistema cruza nome, telefone e aparelho sozinho — conta duplicada é fácil de detectar. As vagas de acesso são limitadas: use sempre o <strong>mesmo usuário</strong> e nunca crie uma segunda conta.","au_f":"✅ 1 pessoa = 1 conta = todos os seus envios e dias de VIP sempre juntos e seguros.", // ⚠️ v149: regra de conta única (ordem do dono)
    "mc_1":"Minha Conta","mc_13":"Comprovante recebido — aguardando confirmação","mc_14":"Renove seu plano na aba Planos.","mc_15":"ATENÇÃO: expira em","mp_pend":"⏳ Em análise","mp_pago":"💰 Pagamento recebido — aguardando confirmação","mp_canc":"✖ Cancelado","mp_canc_sub":"Se achar que foi engano, chame no WhatsApp","mp_motivo":"Motivo","mp_ileg":"⚠️ Não conseguimos ler seu comprovante — reenvie uma foto nítida.","mp_ok":"✅ Comprovante conferido pelo robô","mp_analise":"🔍 Comprovante em análise","mp_reenviar":"📤 Reenviar comprovante","mp_media":"confirmação média real","mp_sub24":"Analisamos e confirmamos — você recebe notificação", "mc_2":"Sem plano ativo","mc_7":"🧾 Último pedido:","mc_8":"aguardando confirmação do admin","mc_9":"confirmado — plano ativado","mc_10":"cancelado","mc_11":"Você ainda não fez nenhum pedido.","mc_12":"Assine um plano pra liberar o envio de candidaturas — manual e automático.", // 👤 2.0-P6: Minha Conta em 5s (v170: sem sistema de diamantes; v172-FIX 12/09: conta sem plano não envia NADA, nunca mais "limite grátis")
    "tut_t":"Central de Tutoriais","tut_s":"20 passo a passos — tudo o que dá pra fazer no H2BApply, explicado tela por tela (vários já com fotos reais)","tut_ph":"🔍 O que você quer aprender? Ex.: currículo, automático, planos...","tut_v":"Nada encontrado com essa palavra — tente outro termo (ex.: \"enviar\", \"plano\", \"perfil\").","tut_load":"⏳ Carregando os tutoriais...", // 📖 v160
    "consent_title":"☑️ Antes de continuar","consent_b1":"É um serviço digital pago de automação de envio — o H2BApply nunca escreve o e-mail por você; o conteúdo enviado é sempre seu.","consent_b2":"O preço mostrado agora é o valor final a pagar.","consent_b3":"Depois de pagar via PIX, envie o comprovante e continue logado na mesma conta até a confirmação — automática quando bate, e sempre conferida pela nossa equipe.","consent_b4":"O risco de bloqueio da conta Gmail é do Google (terceiro), não do H2BApply — dá pra reduzir usando mais de uma conta de envio.","consent_b5":"Você pode acompanhar o status do seu pedido a qualquer momento na aba 🧾 Meus pedidos.","consent_check":"Li e entendo como o programa funciona e o que estou pagando.","consent_terms_link":"Ver Termos completos →","consent_required_msg":"Marque que você leu e entende antes de continuar.","consent_choose_plan":"Escolha um plano e o período antes de continuar.","plan_step1_title":"Escolha seu plano e período","plan_step1_intro":"Você paga <strong>diretamente</strong> pelo plano escolhido. Veja os limites de cada plano, escolha o período e o preço final aparece na hora, antes de pagar.","plan_calc_empty":"Escolha um plano e o período","plan_error_load":"Não deu pra carregar os planos.","plan_try_again":"Tentar de novo","plan_auto_faq_q":"🤖 Como funciona o Envio Automático?","plan_auto_faq_a":"O robô manda as candidaturas sozinho, pelo seu próprio Gmail, usando o currículo e os textos que você mesmo escreveu no seu perfil — ele nunca inventa nada. Esse recurso vem incluído nos planos Turbo e Máximo desde o primeiro dia (o Manual não tem automático: você escolhe e envia cada candidatura). Pra usar o automático, você escolhe a fonte de vagas, ajusta os filtros e escolhe o perfil, toca em Iniciar e pode até fechar o app: ele continua enviando sozinho, um a cada ~7 minutos (ritmo humano, pra proteger sua conta Gmail de bloqueio pelo Google), até bater o limite diário do seu plano — aí ele pausa sozinho e volta a enviar no dia seguinte.","plan_faq_title":"❓ Perguntas frequentes","plan_faq1_q":"Qual a diferença entre os 3 planos?","plan_faq1_a":"Manual dá só candidaturas manuais (você escolhe e envia uma por uma). Turbo soma o robô automático ao manual. Máximo tem o maior volume automático (300/dia) com 2 contas Gmail, mas menos manuais (50/dia) — pensado pra quem quer o máximo volume via robô.","plan_faq2_q":"Por que o plano de 60 dias é mais barato por dia?","plan_faq2_a":"O período mais longo já vem com desconto embutido — você paga menos por dia sem perder nenhum limite diário. O preço exato de cada opção aparece antes de você pagar, direto da nossa tabela oficial.","plan_faq3_q":"O que acontece quando meu plano vencer?","plan_faq3_a":"O envio para automaticamente no dia em que vence — nunca cobramos de novo sozinhos. Quando quiser continuar, é só escolher um plano de novo aqui na aba Planos.","plan_faq4_q":"Posso cancelar e ser reembolsado?","plan_faq4_a":"Sim: você tem até 7 dias corridos após o pagamento pra desistir e pedir reembolso (direito de arrependimento do Código de Defesa do Consumidor). Se o plano já foi usado nesse período, o reembolso é proporcional ao que sobrou. Depois dos 7 dias, não há reembolso automático — veja os Termos completos.","plan_faq5_q":"O envio automático é seguro pro meu Gmail?","plan_faq5_a":"O robô manda num ritmo humano (~7 minutos entre envios) pra reduzir o risco de bloqueio do Google. No plano Máximo, os envios revezam entre 2 contas Gmail, o que reduz ainda mais o volume por conta. Ainda assim, o risco de bloqueio é do Google (terceiro) — nunca do H2BApply.","plan_faq6_q":"Vocês garantem que vou conseguir um emprego?","plan_faq6_a":"Não. O H2BApply é a ponte até o empregador — nunca o autor da sua candidatura nem garantidor do resultado. Contratação, resposta de empresas, aprovação de visto e entrada nos EUA dependem só de empregadores e autoridades americanas, fora do nosso controle.", // 💎 v170: compra direta de plano — consentimento informado (CEO mode, 09/09/2026); v172-FIX 12/09: removida menção a "moeda intermediária" (referência ao sistema antigo já retirado) + explicação do automático reorganizada (dono: "ficou um pouco confuso")
    ...
    "home":"Home","auto":"Auto",
    // Sidebar
    "notifications":"Notificações","plans":"💎 Planos","profile":"Perfil","admin":"Painel Admin","logout":"Sair","logout_confirm":"Sair da sua conta? Você pode entrar de novo a qualquer momento com seu usuário e senha.", // v167: botão de sair (achado E2E crítico)
    // Home
    "auto_title":"Envio Automático","auto_sub":"Toque para configurar e iniciar",
    "manual":"Manual","search":"Pesquisar","responses":"Respostas","auto_lbl":"Automático",
    "me":"Eu","profiles":"Currículos",
    // Plans
    ...
    "extra_gmail":"Gmail Extra para Envios","upgrade_to_enable":"Assine um plano para ativar.",
    ...
    "upsell_cta":"Ver planos e assinar","upsell_later":"Continuar amanhã",
    ...
    "queue_progress":"Progresso da fila","last_sends":"Últimos envios",
    ...
    "q_ph":"Cargo, empresa...",
    "manual_today":"Envios manuais hoje","send_by":"📧 Enviar por","resume_lbl":"📄 Currículo",
    "sending":"Enviando...","optional_lbl":"(opcional)","add_new":"+ Adicionar",
    ...
    "inicio":"Início","auto_send":"Envio Automático",
    ...
    "all":"Todas","random":"🔀 Aleatório","recent":"Recentes",
    ...
    "close":"Fechar","delete":"Excluir",
    // Sys notif
  },
  en: {
     // 🌐 v137b
    "ns0_t":"Pick your plan to start","ns0_s":"Without an active plan, sending is blocked \u2014 manual and automatic.","ns0_c":"See plans","ha_semplano_t":"\u2b50 Pick your plan","ha_semplano_s":"Sending applications (manual and automatic) requires an active plan.","po_pend":"Order under review","po_pend_t":"under review","po_pend_s":"Sent \u00b7 confirmed after admin review","po_canc":"Order cancelled","po_canc_t":"was not accepted","po_canc_s":"You can place a new order with the correct amount.","po_motivo":"Reason","po_refazer":"Place a new order","po_wa":"Chat on WhatsApp","po_fechar":"Dismiss notice","pd_conf":"\u2705 Confirm your contact details","pd_errado":"Something wrong?","pd_corrigir":"Fix it in my Profile","pd_nome":"Name","pd_wpp":"WhatsApp","pd_cidade":"City","pd_toast":"Update your details here, then go back to Plans to finish the order.",
    "ns1_t":"Create your application profile","ns1_s":"It's what goes in the emails to companies. Takes 1 minute.","ns1_c":"Create profile","ns2_t":"Attach your resume (PDF)","ns2_s":"Without a resume attached, your applications won't go out.","ns2_c":"Attach","ns3_t":"All set! Start applying","ns3_s":"Your profile is complete. Send your first application today.","ns3_c":"Find jobs","ns4_t":"You hit today's limit","ns4_s":"Go VIP and send up to 100 applications a day.","ns4_c":"See plans","ns5_t":"Turn on Auto Send","ns5_s":"Let the system send applications while you work.","ns5_c":"Turn on","logs_none":"No logs yet","logs_none_s":"Logs show up here once you use Auto Send", // 🌐 v137: dinâmicos da varredura E2E
    "g_1":"Auto Send","g_2":"Set it up and let the system work for you","g_7":"Resumes","g_30":"your resume (PDF)","g_31":"the email text","g_32":"up to 2 profiles: one H-2B and one H-2A","g_33":"Attaches your PDF resume to every application","g_34":"Sets the email text in English","g_35":"Essential for Auto Send to work","g_36":"Resume used:","g_39":"No expiration · 450 sends/day per connected Gmail · custom interval","roi_vipro":"Turbo — 200/day (manual + automatic)","roi_dp":"Máximo — 350/day (manual + automatic)","pf_gmail_extra_max":"🔒 Extra Gmail — Máximo only","pf_vip_infinito":"Unlimited VIP","g_40":"seconds between each send","g_41":"3min (default)","g_42":"Add Gmails above to set limits","g_44":"Warning:","gmail_contas":"Sending many applications from a single Gmail can get it temporarily blocked by Google. VIP and Turbo send from the Gmail on your account (1 address); Máximo rotates between 2 Gmails, which lowers the volume per account and the blocking risk. The blocking risk is the","g_46":"user's responsibility","g_49":"jobs for you","g_50":"\ud83c\udfaf Your profile will be used in every send:","g_51":"\u25b6 Start now","g_52":"Sends non-stop 24/7. Resets at midnight and keeps going until the queue is empty.","g_53":"\ud83d\udd50 Schedule hours","g_54":"Sends from X to Y o'clock every day. Outside that window it pauses.","g_55":"Start at","g_56":"Stop at","g_62":"Payment via PIX","g_63":"*required","g_64":"Screenshot of your PIX payment confirmation","g_65":"Tap to select the receipt","g_66":"JPG, PNG, PDF \u2014 max 5MB","g_68":"Questions? Contact us:","g_69":"1 company saying yes = you're in the USA \u2708\ufe0f","g_128":"disappear from anywhere public","g_129":"log in again with the same email","g_150":"This profile will be used for manual and automatic sends","g_151":"resume (PDF)","g_152":"\u2460 Basic Info","g_154":"Pick an icon for the profile:","g_155":"\u2461 Resume &amp; Cover Letter (PDF)","g_156":"\ud83d\udcc4 Resume (PDF)","g_157":"\u2705 Resume linked to this profile","g_158":"\ud83d\udce4 New file \u2014 will upload when you save","g_159":"Click or drag a PDF to upload","g_160":"Max 5MB","g_161":"or pick one from your account","g_163":"only for jobs matching this profile's visa type","g_166":"Variables:","g_168":"\u2699\ufe0f Variables \u2014 click to copy:","g_170":"Select the categories this profile will be used for automatically","g_171":"\u2464 Settings","g_172":"Always-on protections:","g_173":"cannot be turned off","g_178":"Only the Máximo plan can connect a 2nd sending Gmail (the robot rotates between both and lowers blocking risk). VIP and Turbo use 1 e-mail; without a plan you can't link a Gmail.","g_188":". You can have","g_189":"the profile you pick in Step 3 of the wizard","g_190":"Create your first Resume Profile to","g_191":"start sending applications","g_193":"Sending Gmails","g_194":"10 FREE sends/day for everyone!","g_198":"\ud83d\udcec Upcoming applications","g_199":"\u2014 Summer","g_202":"Sending alternates","g_203":"Auto queue hit zero? Reset sent","g_204":"You only need to be logged in once.","g_205":"Time (Bras\u00edlia)","g_207":"🧾 My orders","g_210":"📸 Payment receipt","g_211":"📅 Date you paid","g_212":". That's how we verify your payment.","g_239":"Settings","g_239b":"Session","g_239c":"More","tut_center":"Tutorial Center","settings_adv":"Advanced settings","g_241":"Danger zone","g_256":"Compatible sheets","g_257":"Job categories","pe_eng_lbl":"🗣️ Your English (optional)","pe_eng_none":"None","pe_eng_basic":"Basic","pe_eng_inter":"Intermediate","pe_eng_adv":"Advanced","pe_cnh_lbl":"I have a driver's license","pe_cover_note":"📝 The <strong>cover letter</strong> is <strong>NOT required</strong> — your <strong>résumé</strong> (the PDF right above) is enough to apply. If you have one it goes along and may help; if not, just skip this part.","pe_subj_title":"③ Email title (subject)","pe_subj_help":"The <strong>title</strong> is the line the employer sees in the inbox before opening — short and direct, in English, e.g. <em>“Application for {vaga} — {nome}”</em>. Write <strong style='color:var(--red)'>3 different versions</strong> (the 3 fields are already open; you can add more). The system rotates them on every send so it never looks like spam.","pe_count0":"0 of 3 filled in","pe_subj_add":"Add another title","pe_subj_warn":"Write at least <strong>3 different titles</strong> (the fields marked *) — that's the protection against spam blocks.","pe_body_title":"④ Email body (the message)","pe_body_help":"The <strong>body</strong> is the full message the employer reads: who you are, the job you want, your experience and how to reach you. In English, in your own words. Write <strong style='color:var(--red)'>3 different versions</strong> (the 3 fields are already open); the variables below are replaced automatically on every send.","pe_body_add":"Add another text","pe_body_warn":"Write at least <strong>3 different texts</strong> (the fields marked *) — that's the protection against spam blocks.", // 🌐 v136: varredura final (auto)
    ...
    "au_t":"One-account rule — read before signing in","au_b":"Each person may have one H2BApply account. Creating a second account — even with a different e-mail — can lead to a <strong>permanent ban of both accounts</strong>, with nothing refunded. Remember: the name on your resume is always the same, and the system cross-checks name, phone and device on its own — a duplicate account is easy to detect. Access spots are limited: always use the <strong>same username</strong> and never create a second account.","au_f":"✅ 1 person = 1 account = all your sends and VIP days always together and safe.", // ⚠️ v149: regra de conta única (ordem do dono)
    "mc_1":"My Account","mc_13":"Receipt received — awaiting confirmation","mc_14":"Renew your plan in the Plans tab.","mc_15":"ATTENTION: expires in","mp_pend":"⏳ Under review","mp_pago":"💰 Payment received — awaiting confirmation","mp_canc":"✖ Cancelled","mp_canc_sub":"If you think it was a mistake, message us on WhatsApp","mp_motivo":"Reason","mp_ileg":"⚠️ We could not read your receipt — resend a clear photo.","mp_ok":"✅ Receipt verified by the robot","mp_analise":"🔍 Receipt under review","mp_reenviar":"📤 Resend receipt","mp_media":"real average confirmation","mp_sub24":"We review and confirm — you will get a notification", "mc_2":"No active plan","mc_7":"🧾 Last order:","mc_8":"awaiting admin confirmation","mc_9":"confirmed — plan activated","mc_10":"cancelled","mc_11":"You haven't placed any orders yet.","mc_12":"Subscribe to a plan to unlock sending applications — manual and automatic.", // 👤 2.0-P6 (v170: no more diamond system; v172-FIX 09/12: no-plan accounts send NOTHING, never a "free limit" again)
    "tut_t":"Tutorials Center","tut_s":"20 step-by-step guides — everything you can do on H2BApply, screen by screen (several already with real screenshots)","tut_ph":"🔍 What do you want to learn? E.g.: resume, automatic, plans...","tut_v":"Nothing found — try another word (e.g. \"send\", \"plan\", \"profile\").","tut_load":"⏳ Loading tutorials...", // 📖 v160
    "consent_title":"☑️ Before you continue","consent_b1":"This is a paid digital application-sending automation service — H2BApply never writes the e-mail for you; the content sent is always your own.","consent_b2":"The price shown now is the final amount to pay.","consent_b3":"After paying via PIX, send the receipt and stay signed in on the same account until confirmation — automatic when it matches, and always checked by our team.","consent_b4":"The risk of your Gmail account being blocked belongs to Google (a third party), not H2BApply — you can lower it by using more than one sending account.","consent_b5":"You can check your order status anytime in the 🧾 My orders tab.","consent_check":"I have read and understand how the program works and what I'm paying for.","consent_terms_link":"See full Terms →","consent_required_msg":"Check that you've read and understood before continuing.","consent_choose_plan":"Choose a plan and the period before continuing.","plan_step1_title":"Choose your plan and period","plan_step1_intro":"You pay <strong>directly</strong> for the plan you choose. See each plan's limits, pick the period, and the final price shows right away, before you pay.","plan_calc_empty":"Choose a plan and the period","plan_error_load":"Couldn't load the plans.","plan_try_again":"Try again","plan_auto_faq_q":"🤖 How does Automatic Sending work?","plan_auto_faq_a":"The robot sends applications by itself, through your own Gmail, using the resume and texts you wrote yourself in your profile — it never makes anything up. This feature is included in the Turbo and Máximo plans from day one (Manual has no automatic sending: you pick and send each application). To use automatic sending, you pick the job source, adjust the filters and pick your profile, tap Start, and you can even close the app: it keeps sending on its own, one every ~7 minutes (human pace, to protect your Gmail from being blocked by Google), until it hits your plan's daily limit — then it pauses on its own and resumes the next day.","plan_faq_title":"❓ Frequently asked questions","plan_faq1_q":"What's the difference between the 3 plans?","plan_faq1_a":"Manual gives you manual applications only (you pick and send one at a time). Turbo adds the automatic robot on top of manual. Máximo has the highest automatic volume (300/day) with 2 Gmail accounts, but fewer manuals (50/day) — built for whoever wants maximum volume through the robot.","plan_faq2_q":"Why is the 60-day plan cheaper per day?","plan_faq2_a":"The longer period already comes with a built-in discount — you pay less per day without losing any daily limit. The exact price of each option shows up before you pay, straight from our official table.","plan_faq3_q":"What happens when my plan expires?","plan_faq3_a":"Sending stops automatically on the day it expires — we never charge you again on our own. When you want to continue, just pick a plan again here in the Plans tab.","plan_faq4_q":"Can I cancel and get a refund?","plan_faq4_a":"Yes: you have up to 7 calendar days after payment to withdraw and request a refund (right of withdrawal under Brazil's consumer protection code). If the plan was already used during that period, the refund is proportional to what's left. After 7 days, there's no automatic refund — see the full Terms.","plan_faq5_q":"Is automatic sending safe for my Gmail?","plan_faq5_a":"The robot sends at a human pace (~7 minutes between sends) to reduce the risk of a Google block. On the Máximo plan, sends rotate between 2 Gmail accounts, further reducing the volume per account. Even so, the block risk belongs to Google (a third party) — never to H2BApply.","plan_faq6_q":"Do you guarantee I'll get a job?","plan_faq6_a":"No. H2BApply is the bridge to the employer — never the author of your application nor a guarantor of the outcome. Hiring, employer responses, visa approval and entry into the US depend solely on employers and US authorities, outside our control.", // 💎 v170: compra direta de plano — consentimento informado (CEO mode, 09/09/2026); v172-FIX 12/09: removed "middleman currency" mention (old removed system) + reworded automatic explanation
     // 🎯 v139: Vagas pra você (Home)
    ...
    "extra_gmail":"Extra Gmail for Sending","upgrade_to_enable":"Subscribe to a plan to enable it.",
    ...
    "upsell_cta":"See plans & subscribe","upsell_later":"Continue tomorrow",
    ...
    "queue_progress":"Queue progress","last_sends":"Latest sends",
    ...
    "q_ph":"Job title, company...",
    "manual_today":"Manual sends today","send_by":"📧 Send from","resume_lbl":"📄 Resume",
    "sending":"Sending...","optional_lbl":"(optional)","add_new":"+ Add",
    ...
    "inicio":"Home","auto_send":"Auto Send",
    ...
    "all":"All","random":"🔀 Random","recent":"Recent",
  ...
  es: {
     // 🌐 v137b
    "ns0_t":"Elige tu plan para empezar","ns0_s":"Sin plan activo el env\u00edo queda bloqueado \u2014 manual y autom\u00e1tico.","ns0_c":"Ver planes","ha_semplano_t":"\u2b50 Elige tu plan","ha_semplano_s":"Enviar postulaciones (manual y autom\u00e1tico) requiere un plan activo.","po_pend":"Pedido en revisi\u00f3n","po_pend_t":"en revisi\u00f3n","po_pend_s":"Enviado \u00b7 confirmaci\u00f3n tras la revisi\u00f3n del admin","po_canc":"Pedido cancelado","po_canc_t":"no fue aceptado","po_canc_s":"Puedes hacer un pedido nuevo con el valor correcto.","po_motivo":"Motivo","po_refazer":"Hacer nuevo pedido","po_wa":"Hablar por WhatsApp","po_fechar":"Cerrar aviso","pd_conf":"\u2705 Confirma tus datos de contacto","pd_errado":"\u00bfAlg\u00fan dato est\u00e1 mal?","pd_corrigir":"Corregir en mi Perfil","pd_nome":"Nombre","pd_wpp":"WhatsApp","pd_cidade":"Ciudad","pd_toast":"Actualiza tus datos aqu\u00ed y vuelve a Planes para concluir el pedido.",
    "ns1_t":"Crea tu perfil de postulaci\u00f3n","ns1_s":"Es lo que va en los correos a las empresas. Toma 1 minuto.","ns1_c":"Crear perfil","ns2_t":"Adjunta tu curr\u00edculum (PDF)","ns2_s":"Sin curr\u00edculum adjunto, tus postulaciones no salen.","ns2_c":"Adjuntar","ns3_t":"\u00a1Todo listo! Empieza a postularte","ns3_s":"Tu perfil est\u00e1 completo. Env\u00eda tu primera postulaci\u00f3n hoy.","ns3_c":"Buscar empleos","ns4_t":"Alcanzaste el l\u00edmite de hoy","ns4_s":"Hazte VIP y env\u00eda hasta 100 postulaciones al d\u00eda.","ns4_c":"Ver planes","ns5_t":"Activa el Env\u00edo Autom\u00e1tico","ns5_s":"Deja que el sistema env\u00ede postulaciones mientras trabajas.","ns5_c":"Activar","logs_none":"Sin registros todav\u00eda","logs_none_s":"Los registros aparecen aqu\u00ed cuando uses el Env\u00edo Autom\u00e1tico", // 🌐 v137: dinâmicos da varredura E2E
    "g_1":"Env\u00edo Autom\u00e1tico","g_2":"Configura y deja que el sistema trabaje por ti","g_7":"Curr\u00edculums","g_30":"tu curr\u00edculum (PDF)","g_31":"el texto del correo","g_32":"hasta 2 perfiles: uno H-2B y uno H-2A","g_33":"Adjunta tu curr\u00edculum PDF en cada postulaci\u00f3n","g_34":"Define el texto del correo en ingl\u00e9s","g_35":"Esencial para que el Env\u00edo Autom\u00e1tico funcione","g_36":"Curr\u00edculum usado:","g_39":"Sin expiración · 450 envíos/día por Gmail conectado · intervalo configurable","roi_vipro":"Turbo — 200/día (manual + automático)","roi_dp":"Máximo — 350/día (manual + automático)","pf_gmail_extra_max":"🔒 Gmail Extra — exclusivo Máximo","pf_vip_infinito":"VIP Infinito","g_40":"segundos entre cada env\u00edo","g_41":"3min (predeterminado)","g_42":"Agrega Gmails arriba para configurar l\u00edmites","g_44":"Atenci\u00f3n:","gmail_contas":"Enviar muchas candidaturas desde un solo Gmail puede generar bloqueo temporal de Google. VIP y Turbo envían desde el Gmail de tu cuenta (1 dirección); Máximo alterna entre 2 Gmails, lo que reduce el volumen por cuenta y el riesgo de bloqueo. El riesgo de bloqueo es","g_46":"responsabilidad del usuario","g_49":"empleos para ti","g_50":"\ud83c\udfaf Tu perfil se usar\u00e1 en todos los env\u00edos:","g_51":"\u25b6 Empezar ahora","g_52":"Env\u00eda sin parar 24/7. Se reinicia a medianoche y sigue hasta vaciar la cola.","g_53":"\ud83d\udd50 Programar horario","g_54":"Env\u00eda de X a Y horas cada d\u00eda. Fuera del horario queda en pausa.","g_55":"Iniciar a las","g_56":"Parar a las","g_62":"Pago vía PIX","g_63":"*obligatorio","g_64":"Captura de la confirmación del PIX de tu pago","g_65":"Toca para seleccionar el comprobante","g_66":"JPG, PNG, PDF \u2014 m\u00e1x 5MB","g_68":"\u00bfDudas? Cont\u00e1ctanos:","g_69":"1 empresa que confirme = est\u00e1s en EE.UU. \u2708\ufe0f","g_128":"desaparecer de cualquier lugar p\u00fablico","g_129":"iniciar sesi\u00f3n de nuevo con el mismo correo","g_150":"Este perfil se usar\u00e1 en los env\u00edos manuales y autom\u00e1ticos","g_151":"curr\u00edculum (PDF)","g_152":"\u2460 Informaci\u00f3n B\u00e1sica","g_154":"Elige un \u00edcono para el perfil:","g_155":"\u2461 Curr\u00edculum &amp; Cover Letter (PDF)","g_156":"\ud83d\udcc4 Curr\u00edculum (PDF)","g_157":"\u2705 Curr\u00edculum vinculado a este perfil","g_158":"\ud83d\udce4 Archivo nuevo \u2014 se subir\u00e1 al guardar","g_159":"Haz clic o arrastra un PDF para subirlo","g_160":"M\u00e1x. 5MB","g_161":"o elige uno de tu cuenta","g_163":"solo en empleos del tipo de visa de este perfil","g_166":"Variables:","g_168":"\u2699\ufe0f Variables \u2014 clic para copiar:","g_170":"Selecciona las categor\u00edas para las que este perfil se usar\u00e1 autom\u00e1ticamente","g_171":"\u2464 Configuraci\u00f3n","g_172":"Protecciones siempre activas:","g_173":"no se pueden desactivar","g_178":"Solo el plan Máximo puede conectar un 2º Gmail de envío (el robot rota entre los dos y reduce el riesgo de bloqueo). VIP y Turbo usan 1 e-mail; sin plan no se puede vincular Gmail.","g_188":". Puedes tener","g_189":"el perfil que elijas en el Paso 3 del asistente","g_190":"Crea tu primer Perfil de Curr\u00edculum para","g_191":"empezar a enviar postulaciones","g_193":"Gmails de env\u00edo","g_194":"\u00a110 env\u00edos GRATIS/d\u00eda para todos!","g_198":"\ud83d\udcec Pr\u00f3ximas postulaciones","g_199":"\u2014 Verano","g_202":"El env\u00edo alterna","g_203":"\u00bfEl autom\u00e1tico vaci\u00f3 la cola? Restablecer enviados","g_204":"Solo necesitas iniciar sesi\u00f3n una vez.","g_205":"Horario (Bras\u00edlia)","g_207":"🧾 Mis pedidos","g_210":"📸 Comprobante del pago","g_211":"📅 Fecha en que pagaste","g_212":". Con ella verificamos tu pago.","g_239":"Configuraci\u00f3n","g_239b":"Sesi\u00f3n","g_239c":"M\u00e1s","tut_center":"Centro de Tutoriales","settings_adv":"Configuraci\u00f3n avanzada","g_241":"Zona de peligro","g_256":"Planillas compatibles","g_257":"Categor\u00edas de empleo","pe_eng_lbl":"🗣️ Tu inglés (opcional)","pe_eng_none":"No hablo","pe_eng_basic":"Básico","pe_eng_inter":"Intermedio","pe_eng_adv":"Avanzado","pe_cnh_lbl":"Tengo licencia de conducir","pe_cover_note":"📝 La <strong>cover letter</strong> (carta de presentación) <strong>NO es obligatoria</strong> — solo el <strong>résumé</strong> (tu currículum en PDF, arriba) ya basta para postularte. Si tienes una, va junto y puede ayudar; si no, salta esta parte sin problema.","pe_subj_title":"③ Título del e-mail (asunto)","pe_subj_help":"El <strong>título</strong> es la línea que el empleador ve en la bandeja de entrada antes de abrir — corto y directo, en inglés, ej.: <em>“Application for {vaga} — {nome}”</em>. Escribe <strong style='color:var(--red)'>3 versiones diferentes</strong> (los 3 campos ya están abiertos; puedes agregar más). El sistema las alterna en cada envío para no parecer spam.","pe_count0":"0 de 3 completados","pe_subj_add":"Agregar otro título","pe_subj_warn":"Escribe al menos <strong>3 títulos diferentes</strong> (los campos con *) — es la protección contra bloqueos por spam.","pe_body_title":"④ Cuerpo del e-mail (el mensaje)","pe_body_help":"El <strong>cuerpo</strong> es el mensaje completo que el empleador lee al abrir: quién eres, el puesto que quieres, tu experiencia y cómo contactarte. En inglés, con tus palabras. Escribe <strong style='color:var(--red)'>3 versiones diferentes</strong> (los 3 campos ya están abiertos); las variables de abajo se reemplazan automáticamente en cada envío.","pe_body_add":"Agregar otro texto","pe_body_warn":"Escribe al menos <strong>3 textos diferentes</strong> (los campos con *) — es la protección contra bloqueos por spam.", // 🌐 v136: varredura final (auto)
    ...
    "au_t":"Regla de cuenta única — lee antes de entrar","au_b":"Cada persona puede tener una cuenta en H2BApply. Crear una segunda cuenta — incluso con otro e-mail — puede causar un <strong>baneo permanente de las dos cuentas</strong>, sin devolución de nada. Recuerda: el nombre en tu currículum es siempre el mismo, y el sistema cruza nombre, teléfono y dispositivo por sí solo — una cuenta duplicada es fácil de detectar. Los cupos de acceso son limitados: usa siempre el <strong>mismo usuario</strong> y nunca crees una segunda cuenta.","au_f":"✅ 1 persona = 1 cuenta = todos tus envíos y días de VIP siempre juntos y seguros.", // ⚠️ v149: regra de conta única (ordem do dono)
    "mc_1":"Mi Cuenta","mc_13":"Comprobante recibido — esperando confirmación","mc_14":"Renueva tu plan en la pestaña Planes.","mc_15":"ATENCIÓN: expira en","mp_pend":"⏳ En revisión","mp_pago":"💰 Pago recibido — esperando confirmación","mp_canc":"✖ Cancelado","mp_canc_sub":"Si crees que fue un error, escríbenos por WhatsApp","mp_motivo":"Motivo","mp_ileg":"⚠️ No pudimos leer tu comprobante — reenvía una foto nítida.","mp_ok":"✅ Comprobante verificado por el robot","mp_analise":"🔍 Comprobante en revisión","mp_reenviar":"📤 Reenviar comprobante","mp_media":"confirmación media real","mp_sub24":"Revisamos y confirmamos — recibirás una notificación", "mc_2":"Sin plan activo","mc_7":"🧾 Último pedido:","mc_8":"esperando confirmación del admin","mc_9":"confirmado — plan activado","mc_10":"cancelado","mc_11":"Aún no hiciste ningún pedido.","mc_12":"Suscríbete a un plan para desbloquear el envío de solicitudes — manual y automático.", // 👤 2.0-P6 (v170: sin sistema de diamantes; v172-FIX 12/09: cuenta sin plan no envía NADA, nunca más un "límite gratis")
    "tut_t":"Central de Tutoriales","tut_s":"20 guías paso a paso — todo lo que puedes hacer en H2BApply, pantalla por pantalla (varias ya con fotos reales)","tut_ph":"🔍 ¿Qué quieres aprender? Ej.: currículum, automático, planes...","tut_v":"Nada encontrado — prueba otra palabra (ej. \"enviar\", \"plan\", \"perfil\").","tut_load":"⏳ Cargando los tutoriales...", // 📖 v160
    "consent_title":"☑️ Antes de continuar","consent_b1":"Es un servicio digital pago de automatización de envío — H2BApply nunca escribe el correo por ti; el contenido enviado siempre es tuyo.","consent_b2":"El precio mostrado ahora es el valor final a pagar.","consent_b3":"Después de pagar vía PIX, envía el comprobante y sigue conectado en la misma cuenta hasta la confirmación — automática cuando coincide, y siempre revisada por nuestro equipo.","consent_b4":"El riesgo de bloqueo de la cuenta Gmail es de Google (tercero), no de H2BApply — se reduce usando más de una cuenta de envío.","consent_b5":"Puedes revisar el estado de tu pedido en cualquier momento en la pestaña 🧾 Mis pedidos.","consent_check":"Leí y entiendo cómo funciona el programa y qué estoy pagando.","consent_terms_link":"Ver Términos completos →","consent_required_msg":"Marca que leíste y entendiste antes de continuar.","consent_choose_plan":"Elige un plan y el período antes de continuar.","plan_step1_title":"Elige tu plan y período","plan_step1_intro":"Pagas <strong>directamente</strong> por el plan elegido. Mira los límites de cada plan, elige el período y el precio final aparece al instante, antes de pagar.","plan_calc_empty":"Elige un plan y el período","plan_error_load":"No se pudieron cargar los planes.","plan_try_again":"Intentar de nuevo","plan_auto_faq_q":"🤖 ¿Cómo funciona el Envío Automático?","plan_auto_faq_a":"El robot envía las solicitudes solo, por tu propio Gmail, usando el currículum y los textos que tú mismo escribiste en tu perfil — nunca inventa nada. Esta función viene incluida en los planes Turbo y Máximo desde el primer día (el Manual no tiene automático: eliges y envías cada solicitud). Para usar el automático, eliges la fuente de vacantes, ajustas los filtros y eliges tu perfil, tocas Iniciar y puedes hasta cerrar la app: sigue enviando solo, uno cada ~7 minutos (ritmo humano, para proteger tu cuenta Gmail de un bloqueo de Google), hasta llegar al límite diario de tu plan — ahí se pausa solo y vuelve a enviar al día siguiente.","plan_faq_title":"❓ Preguntas frecuentes","plan_faq1_q":"¿Cuál es la diferencia entre los 3 planes?","plan_faq1_a":"Manual da solo candidaturas manuales (eliges y envías una por una). Turbo suma el robot automático al manual. Máximo tiene el mayor volumen automático (300/día) con 2 cuentas de Gmail, pero menos manuales (50/día) — pensado para quien quiere el máximo volumen vía robot.","plan_faq2_q":"¿Por qué el plan de 60 días es más barato por día?","plan_faq2_a":"El período más largo ya viene con descuento incluido — pagas menos por día sin perder ningún límite diario. El precio exacto de cada opción aparece antes de pagar, directo de nuestra tabla oficial.","plan_faq3_q":"¿Qué pasa cuando mi plan vence?","plan_faq3_a":"El envío se detiene automáticamente el día que vence — nunca cobramos de nuevo por nuestra cuenta. Cuando quieras continuar, solo elige un plan de nuevo aquí en la pestaña Planes.","plan_faq4_q":"¿Puedo cancelar y recibir un reembolso?","plan_faq4_a":"Sí: tienes hasta 7 días corridos después del pago para desistir y pedir reembolso (derecho de arrepentimiento del código de defensa del consumidor). Si el plan ya fue usado en ese período, el reembolso es proporcional a lo que quedó. Después de los 7 días, no hay reembolso automático — mira los Términos completos.","plan_faq5_q":"¿El envío automático es seguro para mi Gmail?","plan_faq5_a":"El robot envía a un ritmo humano (~7 minutos entre envíos) para reducir el riesgo de bloqueo de Google. En el plan Máximo, los envíos rotan entre 2 cuentas de Gmail, reduciendo aún más el volumen por cuenta. Aun así, el riesgo de bloqueo es de Google (un tercero) — nunca de H2BApply.","plan_faq6_q":"¿Garantizan que voy a conseguir un empleo?","plan_faq6_a":"No. H2BApply es el puente hasta el empleador — nunca el autor de tu solicitud ni garante del resultado. La contratación, la respuesta de las empresas, la aprobación de la visa y la entrada a EE. UU. dependen solo de los empleadores y las autoridades estadounidenses, fuera de nuestro control.", // 💎 v170: compra direta de plano — consentimento informado (CEO mode, 09/09/2026); v172-FIX 12/09: quitada mención a "moneda intermediaria" (sistema antiguo ya retirado) + explicación del automático reorganizada
     // 🎯 v139: Vagas pra você (Home)
    ...
    "extra_gmail":"Gmail Extra para Envíos","upgrade_to_enable":"Suscríbete a un plan para activarlo.",
    ...
    "upsell_cta":"Ver planes y suscribirse","upsell_later":"Seguir mañana",
    "st_starting":"🟡 Iniciando...","st_sending":"🟢 Enviando...","st_paused":"⏸ Pausado",
    ...
    "queue_progress":"Progreso de la cola","last_sends":"Últimos envíos",
    ...
    "q_ph":"Puesto, empresa...",
    "manual_today":"Envíos manuales hoy","send_by":"📧 Enviar desde","resume_lbl":"📄 Currículum",
    "sending":"Enviando...","optional_lbl":"(opcional)","add_new":"+ Añadir",
    ...
    "inicio":"Inicio","auto_send":"Envío Automático",
    ...
    "all":"Todas","random":"🔀 Aleatorio","recent":"Recientes",
    ...
    "close":"Cerrar","delete":"Eliminar",
  }

};
// v199 LOTE 17: o app é SÓ em português, de propósito (regra do README/CLAUDE.md:
// "não tem seletor de idioma nem detecta idioma do navegador"). Até aqui a regra
// era declarada mas não era real: `_curLang` aceitava 'en'/'es' vindos do
// localStorage `h2b_lang`, gravado a partir do `d.language` do servidor — e como
// NENHUMA tela tem botão de trocar idioma, uma conta com language='en' (legada ou
// gravada por um POST direto) abria o site inteiro em inglês SEM caminho de volta.
// Agora é uma constante: um valor só, atribuído uma vez. Os dicionários EN/ES
// continuam vivos (e as guardas i18n do smoke junto) — o dia em que existir
// seletor, ele muda esta linha e mais nada.
const _curLang = 'pt';

function t(key){ return (LANG_DICT[_curLang]||LANG_DICT.pt)[key] || (LANG_DICT.pt[key]) || key; }

function applyLang(){
  document.documentElement.lang = 'pt-BR'; // v199 LOTE 17: idioma fixo
  ...
  // Filter buttons (v173: os filtros vivem no painel VF, montado por JS já traduzido)
...
async function loadPlanos(){
  ...
  // seletor de compra. plan-status-card e Meus Pedidos continuam do jeito
  // de sempre — só a compra NOVA some.
...
// v218 (reestruturação de Planos, 20/09/2026): plano "recomendado" em
// destaque (padrão de mercado — pricing page com 3 opções sempre realça a
// do meio) e selo de economia no período mais longo, calculado sempre a
// partir do PREÇO REAL de 30d (nunca um número fixo — se a tabela mudar de
// novo, o selo muda sozinho). Fonte única continua sendo GET /api/planos.
const PLANO_RECOMENDADO='vipro';
function _renderPlanosUI(d){
  ...
  box.innerHTML=['vip','vipro','doublepro'].map(pl=>{
    ...
    // 🎯 (ordem do dono, 12/09/2026 — "deixa mais explícito a quantidade de
    // envios por cada plano") badges grandes e coloridos em vez de 1 linha
    // pequena e cinza — manual (azul) e automático (verde, ou cinza
    // "sem automático" quando o plano não inclui, ex.: Manual).
    const manualBadge=lim.manual
      ?`<span style="display:inline-flex;align-items:center;gap:5px;background:var(--bluel);color:var(--blue);font-size:12.5px;font-weight:800;border-radius:9px;padding:6px 11px">✋ ${lim.manual} manuais/dia</span>`
      :'';
    const autoBadge=lim.auto
      ?`<span style="display:inline-flex;align-items:center;gap:5px;background:var(--greenl);color:var(--green);font-size:12.5px;font-weight:800;border-radius:9px;padding:6px 11px">🤖 ${lim.auto} automáticas/dia</span>`
      :`<span style="display:inline-flex;align-items:center;gap:5px;background:var(--sf2);color:var(--t3);font-size:12px;font-weight:700;border-radius:9px;padding:6px 11px">🤖 Sem envio automático</span>`;
    // 📧 ordem do dono (13/09/2026): quantos e-mails de envio cada plano dá
    // TEM que aparecer pro usuário — Máximo 2 (revezamento), demais 1.
    const emailBadge=`<span style="display:inline-flex;align-items:center;gap:5px;background:var(--purplel);color:var(--purple);font-size:12px;font-weight:800;border-radius:9px;padding:6px 11px">📧 ${esc(pl==='doublepro'?t('plan_emails_2'):t('plan_emails_1'))}</span>`;
    const isSel=sel===pl;
    const isReco=pl===PLANO_RECOMENDADO;
    const borderCor=isSel?'var(--blue)':isReco?'#f59e0b':'var(--border2)';
    const precoBase=(porPlano[pl]||[]).find(c=>c.dias===30);
    return `<div style="position:relative;background:var(--surface);border:2px solid ${borderCor};border-radius:var(--rl);padding:${isReco?'20px 14px 14px':'14px'};margin-bottom:12px;${isSel?'box-shadow:0 0 0 3px rgba(37,99,235,.14)':isReco?'box-shadow:0 0 0 3px rgba(245,158,11,.14)':''}">
      ${isReco?'<div style="position:absolute;top:-11px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#f59e0b,#ea580c);color:#fff;font-size:10.5px;font-weight:800;border-radius:20px;padding:4px 14px;white-space:nowrap;box-shadow:0 3px 8px rgba(245,158,11,.4)">🔥 MAIS POPULAR</div>':''}
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <div style="font-size:14.5px;font-weight:800">${esc(PLAN_NAMES[pl])}</div>
      ...
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px">
        ${(porPlano[pl]||[]).map(c=>{
          ...
          // 💰 Selo "economize" no período mais longo — sempre calculado
          // contra o preço de 30d da tabela (nunca um valor fixo escrito à mão).
          const economia=(c.dias>30&&precoBase)?Math.round((precoBase.valorTotal*(c.dias/30)-c.valorTotal)):0;
          return `<button type="button" class="btn ${chSel?'btn-primary':'btn-secondary'}" style="position:relative;justify-content:center;font-size:12.5px;padding:10px 6px;flex-direction:column;gap:2px;height:auto" onclick="selectPlanoDias('${pl}',${c.dias})">
            <span>${DIAS_LBL[c.dias]||c.dias+'d'} · R$ ${brl(c.valorTotal)}</span>
            ${economia>0?`<span style="font-size:10px;font-weight:800;color:${chSel?'#dcfce7':'var(--green)'}">💰 Economize R$ ${brl(economia)}</span>`:''}
          </button>`;
        }).join('')}
      </div>
    </div>`;
...
function _updatePlanCalcResumo(){
  ...
  if(cDesc)cDesc.textContent=`${PLAN_NAMES[pl]||pl} · ${dias===365?'1 ano':dias+' dias'}`;
...
// ── v185 LOTE 3: dados de contato do passo 2 ────────────────────────────
// O cadastro (v175) JÁ obriga nome, WhatsApp, cidade e estado — pedir tudo
// de novo no meio da compra era digitação à toa (e a cidade nem vinha
// preenchida). Com os 4 dados na conta, o passo 2 vira CONFIRMAÇÃO em
// leitura, com link pra corrigir no Perfil. Conta legada (sem cidade ou sem
// estado) cai no formulário editável — já pré-preenchido com o que existe.
// Nunca travar a compra de quem se cadastrou antes do campo existir.
function _planDadosDaConta(){
  return {
    nome:(U.name||CFG.name||'').trim(),
    wpp:(U.whatsapp||CFG.whatsapp||U.phone||CFG.phone||'').trim(),
    cidade:(CFG.city||'').trim(),
    estado:(CFG.estado||'').trim(),
  };
}
function _planPrepararDados(){
  const dd=_planDadosDaConta();
  const completo=!!(dd.nome&&dd.wpp&&dd.cidade&&dd.estado);
  const boxR=g('#plan-dados-resumo'), boxF=g('#plan-dados-form');
  if(boxR)boxR.style.display=completo?'block':'none';
  if(boxF)boxF.style.display=completo?'none':'flex';
  if(completo){
    const linhas=g('#plan-dados-linhas');
    if(linhas)linhas.innerHTML=[
      [t('pd_nome'),dd.nome],[t('pd_wpp'),dd.wpp],[t('pd_cidade'),dd.cidade+' · '+dd.estado],
    ].map(([k,v])=>`<div style="display:flex;justify-content:space-between;gap:10px"><span style="color:var(--t3)">${esc(k)}</span><strong style="text-align:right">${esc(v)}</strong></div>`).join('');
  }else{
    const set=(id,v)=>{const e=g(id);if(e&&!e.value)e.value=v||'';};
    set('#plan-form-name',dd.nome);set('#plan-form-wpp',dd.wpp);
    set('#plan-form-city',dd.cidade);set('#plan-form-state',dd.estado);
  }
}
function planCorrigirNoPerfil(){
  sv('profile');
  toast(t('pd_toast'),'au',6000);
}
...
function goToPlanStep2() {
  ...
  if(sc) sc.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;font-size:14px">
    <div><strong>${esc(PLAN_NAMES[plano]||plano)}</strong> · ${dias===365?'1 ano':dias+' dias'}</div>
  ...
  if(payEl){
    payEl.innerHTML=`<div style="background:linear-gradient(135deg,rgba(16,185,129,.1),rgba(5,150,105,.06));border:1.5px solid rgba(16,185,129,.35);border-radius:12px;padding:14px">
      <div style="font-size:13px;font-weight:800;color:var(--green);margin-bottom:10px;display:flex;align-items:center;gap:6px">
        📱 Pagar via Pix (PicPay) — ${esc(PLAN_NAMES[plano]||plano)} · ${dias===365?'1 ano':dias+' dias'}
      </div>
      ...
      <div style="background:rgba(16,185,129,.12);border-radius:8px;padding:10px;text-align:center">
        ...
        <div style="font-size:11px;color:var(--t3);margin-top:2px">${esc(PLAN_NAMES[plano]||plano)} · ${dias===365?'1 ano':dias+' dias'}</div>
      </div>
...
// ── ⚡ v185 LOTE 3: a ativação provisória precisa APARECER ───────────────
// Comprovante que confere libera o plano NA HORA no servidor (decisão do
// dono, 21/07) — mas a tela nunca re-consultava o /api/status, então quem
// acabou de comprar clicava em "Ir para Envio Automático" e batia no cadeado
// "Plano necessário". Só um F5 resolvia. Aqui o front confere 3 vezes (5s,
// 15s, 40s) e PARA — nada de setInterval permanente (Render Free, banda e
// CPU). Falha de rede é silenciosa: o texto honesto de "um admin confirma"
// continua valendo. E NUNCA prometemos o provisório de antemão: sem
// GEMINI_API_KEY configurada ele simplesmente não acontece, e prometer o que
// pode não vir é pior que não prometer nada.
function _vigiarAtivacaoProvisoria(planoLabel){
  const tentativas=[5000,15000,40000];
  const tenta=(i)=>{
      ...
      try{
        ...
        if(d&&d.connected){
          ...
          if(!d.needsPlan){
            ...
            if(dMsg)dMsg.innerHTML='O robô conferiu seu comprovante e já liberou o <strong>'+esc(planoLabel)+'</strong> — pode começar a enviar agora mesmo.<br>A confirmação final da equipe continua na fila (isso não muda nada pra você).<br><strong>Para dúvidas, entre em contato:</strong>';
            toast('⚡ Plano liberado — já pode enviar!','g',7000);
            try{renderHome();}catch(e){}
            return; // acabou: não precisa das próximas conferidas
          }
        }
      }catch(e){}
      tenta(i+1);
...
async function submitPlanOrder() {
  ...
  const plano=window._planoEscolhido, dias=window._diasEscolhido;
  // v185 LOTE 3: com os dados completos na conta, o passo 2 é só confirmação
  // (formulário escondido) — o pedido usa o que já está cadastrado. Conta
  // legada sem cidade/estado segue pelo formulário editável.
  const _dadosConta=_planDadosDaConta();
  const _usaResumo=g('#plan-dados-form')?.style.display==='none';
  const name = _usaResumo ? _dadosConta.nome : (g('#plan-form-name')?.value.trim() || '');
  const wpp = _usaResumo ? _dadosConta.wpp : (g('#plan-form-wpp')?.value.trim() || '');
  const city = _usaResumo ? _dadosConta.cidade : (g('#plan-form-city')?.value.trim() || '');
  const state = _usaResumo ? _dadosConta.estado : (g('#plan-form-state')?.value.trim() || '');
    ...
    if(d.ok) {
      ...
      if(d.duplicado) {
        ...
        const medTxt=(typeof med==="number"&&med>0)
          ...
          :'assim que possível';
        if(dMsg) dMsg.innerHTML = 'Recebemos seu pedido e o comprovante.<br>Um admin confirma manualmente — '+medTxt+' — e assim que confirmado, seu <strong>'+(PLAN_NAMES[plano]||plano)+'</strong> é ativado.<br><strong>Para dúvidas, entre em contato:</strong>';
        toast('✅ Pedido enviado! Aguarde a confirmação.', 'g');
        _vigiarAtivacaoProvisoria(PLAN_NAMES[plano]||plano);
      }
